...
- **App nativa portal guardia:** seguimiento en **`docs/MOBILE-GUARDIA-IMPLEMENTACION.md`**. Código: `apps/mobile-guardia/` + `packages/portal-core`. **Portal web del vigilador:** SPA Expo en **`https://comtroldata.web.app/app`** (`npm run build:web` → `dist-web`; deploy-lib copia a `build/hosting/app`). Reemplaza `/empleado/*` (redirects hosting). Sin OTA/EAS en web. iPhone = PWA/Safari en `/app`.
