...
| `automation_runs` | Corridas de automatización operativa (P0 planning, P1 replan diario). Incluye métricas, dryRun y muestra de ítems. |
| `automation_alerts_ledger` | Deduplicación de alertas IA por fingerprint (evita novedades repetidas). |
| `operational_closure_checklists` | Snapshots de checklist de cierre mensual (liquidación/prefactura). |
...
**Escalera vacantes SIN PLANIFICAR** (hueco SLA vs malla; `gestionarVacantes` + monitor Ops):
1. **Al detectar** → materializa vacante accionable (`autodev_*`, `UNCOVERED`, no DEVUELTA) + novedad `VACANTE_A_PLANIFICACION` (dueño temprano: Planificación). Ops sigue viendo CUBRIR.
2. **T−2 h** → alerta Ops `VACANTE_PROTOCOLO_COBERTURA` (sin cascada). Cambio de última hora (&lt;2 h) cae acá de inmediato.
3. **T−1 h / ya iniciado** → cascada Auto (`iniciarCascadaCobertura`) + Demo abre protocolo también para `NO_PLANNING`.
4. **T+2 h** → `SIN_COBERTURA` si sigue abierto.
Ausencias (`VACANTE_POR_AUSENCIA`) siguen el circuito Ops/cascada de siempre (no se “devuelven” a Plan como dueño temprano).

**Análisis — diccionario canónico de KPIs (plan/real):** en Informe, Demanda, Financiera, Calidad de dato y `hours_balances`, el KPI llamado «plan» = **`plan_pub_hs`** (malla publicada, coalesce por celda emp+día, +FT; **sin** borrador ni ext/adel). Nunca mezclar `draft` en ese número. Vocabulario fijo:

| ID | Nombre UI | Definición |
|----|-----------|------------|
| `sla_hs` | Horas vendidas | SLA vigente en el período |
| `plan_pub_hs` | Plan comprometido | Publicado + coalesce + FT; sin ext/adel |
| `plan_total_hs` | Plan grilla | pub + borrador (solo Calidad / avance interno) |
| `ext_adel_hs` | Tramos extra/adelanto | Extensión + adelanto publicados |
| `ops_hs` | Cobertura operativa (hs) | Orígenes RETEN / OPERATIONS_COVERAGE / SLA_VIRTUAL |
| `real_hs` | Horas realizadas | Solo `isShiftFichado` (incluye `presentAt` si la banda ya cerró; en curso no suma al mes) |
| `consumo_hs` | Consumo hs-hombre | `plan_pub` + novedades + ext/adel + ops + idle(F/RET/REF); **FT 1×** (ya en plan) |
| `cobertura_plan_pct` | Cobertura de plan | `plan_pub / sla` |
| `cobertura_real_pct` | Cumplimiento real | `real / sla` |
| `avance_fichada_pct` | Avance de fichada | `real / plan_pub` |

Si hay extracto y la malla aún no cargó: reales se pintan desde `hours_balances.realHours` (nunca 0 falso). Solapa **Herramientas → Calidad de dato** audita % publicada, hs borrador, gap `presentAt`↔`isPresent`, Δ extracto↔malla. Smoke: `npm run eval:analisis-queries` (assert Demanda.plan ≈ Financiera.hsPlan).

**Análisis — snapshot en memoria:** `useAnalisisSnapshot` + `lib/analisis/*`. Catálogo (SLA, empleados, geo, `tipos_novedad`) una vez por empresa; hechos (`turnos` + `ausencias`) = **hot** = mes en curso + **2** meses cerrados (`analisisWorkingWindow` / `dataRetention`). Cambiar día/semana/mes dentro de esa ventana = 0 queries (slice en memoria). Fuera de la ventana (abril, año, etc.) se amplía con merge. Analítica reutiliza el mismo store. Header = **universo real** (clientes, objetivos, puestos/`quantity`, slots SLA por banda, pico simultáneo, plantel) — sin selectores 24/25 ni 8/12. **192 = jornada de referencia** (universo/viabilidad); **200 = techo de liquidación** (`analisisBolsa.ts`). Bolsa realista = plantilla ACTIVE × 200 × (1 − índice ausencia 3 meses cerrados previos); si no hay historial 3m → **sin índice** (se muestra el techo, no se finge capacidad). Solapas: **Operativa** (Informe / Demanda / Cobertura / Capacidad y bandas), **Humana** (Guardias + ART.12; sliders ausentismo = what-if), **Financiera** (hs-hombre: SLA vs consumo plan/real + novedades; FT ya en plan_pub — sin doble-cuenta; **gasto** también suma francos F/FF, RET no activado 8 hs y REF/ESC; pirámide objetivo → cliente → empresa; **solo horas, sin precios**; novedades V/E/L/A/AA/PG/SUS = jornada del turno (8 o 12 hs), nunca 24 hs de calendario; Financiera desglosa SUS (no va a Otr); licencias sin `objectiveId` se atribuyen al último puesto de malla o quedan en `SIN_OBJETIVO`), **Herramientas** (Viabilidad / Analítica / Proyección / **Calidad de dato**). Informe: vendido / plan comprometido / realizado / bolsa; KPIs **Cobertura de plan** vs **Cumplimiento real**; períodos día/semana/mes/trimestre/semestre/año; export Excel. Demanda: SLA vs plan_pub vs resultante (plan + ext/adel + ops). **Cobertura** usa la misma fuente que Demanda (`buildDemandaByObjective` / plan+vacante); el drill código/guardia viene de la malla. **Capacidad y bandas** también pinta plan/vacante desde Demanda. Deep-link a Planificación: `/admin/planificacion/?objectiveId=&clientId=&year=&month=` (mes 1–12). Selector de período se bloquea hasta terminar la carga. Quien cubre suma el turno (M=8); FT/novedades no inflan cobertura operativa aparte del plan_pub. Ausentismo real COSP (`shiftId` / `isAbsent` / `AUTO_T30`). **Carga:** primero el extracto mensual CRM (`hours_balances`, 1 doc por objetivo/mes — mismo que el dashboard: cambio de mes instantáneo). Si hay extracto, Informe/Demanda/Financiera pintan a nivel objetivo **sin esperar la malla**; banner «actualizando malla» hasta que llegan guardias, F/RET/REF y desglose fino. Ausencias RRHH van con el catálogo. Malla por mes calendario (misma query que Planificación); overlay = ventana hot. Al tener malla se reescribe el extracto. Recargar en el header. **KPIs cruzados (Dashboard / CRM / Análisis):** SLA = contrato vigente (`pickVigenteSlasForPeriod` + `slaHoursForServiceInRange`); plan = malla publicada (`isPublishedPlanificadorPlannedHoursShift` + coalesce + `calcPlanningSlaReconciliationHours`); realizadas = fichadas reales (`isShiftFichado` / `fichadaHoursForShift`) — si nadie fichó y no hay extracto queda 0, no se infiere del plan. CRM pisa el SLA del extracto `hours_balances` con el contrato vivo. Smoke: `npm run eval:analisis-queries` (desde `apps/web2`).
...
- **Automatización operativa (P0/P1):** núcleo en `apps/functions/src/automation/`. **P0:** `runPlanningAutomationP0`, `runOperationalAlertsScan`, `runOperationalClosureChecklist` + cron `operationalAlertsCron` (*/15). **P1:** `recommendCoverageCandidatesP1` (ranking CCT costo/riesgo), `runDailyReplanP1` (ventana rolling) + cron `dailyReplanCron` (06:00 AR, solo dryRun si `pilotoAutoEnabled`). UI: Configuración → Salud. Tools asistente: `resumen_alertas_operativas_ia`, `checklist_cierre_operativo_mes`, `recomendar_cobertura_vacante`, `ejecutar_replan_diario`.
...
- **App nativa portal guardia (planificado):** seguimiento por fases y tareas en **`docs/MOBILE-GUARDIA-IMPLEMENTACION.md`**. Al cerrar cada tarea (IDs `F0-01` … `F7-12`) o una fase completa, **actualizar ese archivo** (panel de estado, bitácora, «Qué falta ahora»). Código futuro: `apps/mobile-guardia/` + `packages/portal-core`. El portal web `/empleado/*` se mantiene en paralelo.
