...
**Análisis — snapshot en memoria:** `useAnalisisSnapshot` + `lib/analisis/*`. Catálogo (SLA, empleados, geo, `tipos_novedad`) una vez por empresa; hechos (`turnos` + `ausencias`) con rango envolvente (mes) y merge si se amplía. Día/semana dentro del mes ya cargado = 0 queries. Analítica reutiliza el mismo store. Header = **universo real** (clientes, objetivos, puestos/`quantity`, slots SLA por banda, pico simultáneo, plantel) — sin selectores 24/25 ni 8/12. Bolsa = plantilla ACTIVE × **200 techo** × (1 − **índice ausencia 3 meses cerrados previos**); 200 no es el promedio (`analisisBolsa.ts`). Jornada de referencia 192 queda en universo/proyección. Solapas: **Operativa** (Informe / Demanda / Cobertura / Dist. horas), **Humana** (Guardias + ART.12; sliders ausentismo = what-if), **Financiera** (hs-hombre: SLA vs consumo plan/real + novedades + FT; pirámide objetivo → cliente → empresa; sin precios), **Herramientas** (Viabilidad / Analítica / Proyección). Informe: vendido/plan/realizado/bolsa, novedades, conclusiones; períodos día/semana/mes/trimestre/semestre/año; export Excel. Demanda: SLA vs plan vs resultante (ext/FT/ops). Ausentismo real COSP (`shiftId` / `isAbsent` / `AUTO_T30`). Recargar en el header. Smoke: `npm run eval:analisis-queries` (desde `apps/web2`).
