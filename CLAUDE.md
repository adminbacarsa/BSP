...
- Backend: callable Firebase **`chatPlatformAssistant`** (`apps/functions/src/index.ts` + `assistant/*`) invoca **Gemini** con API key en **servidor**. Con permiso READ y `empresaId` en sesión, herramientas **sólo lectura** Firestore (legajo por nombre; **`consultar_turnos_empleado`** por rango AR; **`resumen_presencias_objetivos_dia`** agregados de presencia/ausencia por día; **`listado_turnos_operativos_dia`** lista de turnos visibles; **`contar_servicios_sla_vigentes_empresa`** cuántos SLA están activos y vigentes en la fecha para clientes de la empresa). En producción: **`timeoutSeconds: 180`**, **`memory: 512MB`**, secreto **`GEMINI_API_KEY`**. El cliente envía **`clientToday`** (YYYY-MM-DD) y **timeout ~210 s** en el `httpsCallable`. Portal **empleado**: sólo turnos propios.
