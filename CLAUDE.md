...
- **CRM — autocompletar cliente por CUIT (AFIP):** callable **`lookupClientByCuit`** (`apps/functions/src/afip/*`, SDK `@afipsdk/afip.js`, servicio Constancia de Inscripción). UI: botón **AFIP** junto al CUIT en nuevo cliente y ficha INFO. Secrets: **`AFIP_CUIT`** (CUIT del certificado), **`AFIP_CERT`**, **`AFIP_PRIVATE_KEY`** (PEM; en `.env` usar `\n` para saltos de línea). **`AFIP_PRODUCTION`**: `true` prod / `false` homologación. Emulador: mismas variables en `apps/functions/.env`. El certificado debe tener autorizado el WS **`ws_sr_constancia_inscripcion`** en AFIP. Crear secrets: `firebase functions:secrets:set AFIP_CUIT` (idem CERT y PRIVATE_KEY). Redeploy con secrets montados: PowerShell `$env:DEPLOY_AFIP_WITH_SECRETS='true'; firebase deploy --only functions:lookupClientByCuit`. Sin secrets la función responde «AFIP no configurado».
---
