...
export const VPLAN_STAGES: VplanStageDef[] = [
  ...
  },
  {
    intent: 'strategy',
    phase: '4_strategy',
    title: 'Estrategia — reglas antes de generar',
    shortLabel: '4 · Estrategia',
    output: 'context.strategy — engine, cycle, modes.useTrailing',
    checks: [
      'engine = FixedBandFloater para 6+2 estándar.',
      'CONTINUE: useTrailing = true y racha N > 0 guardias.',
      'cycle = preferredCycle elegido (6+2 → M/T/N, 4+2 → D12/N12).',
      'Modo GREENFIELD: useTrailing = false (arranque en frío).',
    ],
    recommendedMode: 'CONTINUE',
  ...
  },
  {
    ...
    checks: [
      ...
      'Guardia con puesto+banda fija (defaultShiftByEmp): solo M, T o N asignada en días trabajo.',
      'Resto del subgrupo 24hs rota bandas para cerrar 1M+1T+1N+1F/día por puesto.',
      'CONTINUE: trailing + apertura jul-01 coherente con 30/jun (sin F si racha incompleta).',
      'Si hay FIXED_BAND_MISMATCH: revisar asignación en planificacion_estados; cobertura manda.',
    ],
    recommendedMode: 'CONTINUE',
...
export const VPLAN_VALIDATION_PLAYBOOK = [
  ...
  { step: 3, intent: 'generate' as VplanIntent, mode: 'CONTINUE' as VplanRunMode, note: 'Inspeccionar grilla sin verify aún' },
  { step: 4, intent: 'verify' as VplanIntent, mode: 'CONTINUE' as VplanRunMode, note: 'Cobertura y transiciones' },
  { step: 5, intent: 'full' as VplanIntent, mode: 'CONTINUE' as VplanRunMode, note: 'Solo si 4 pasó' },
