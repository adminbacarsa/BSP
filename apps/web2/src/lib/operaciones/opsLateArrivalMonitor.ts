...
export type OpsLateArrivalMonitorInput = {
  ...
  startMs: number;
  nowMs: number;
  /** Turno elegible (no franco, no presente, no ausente confirmado, etc.) */
  eligible: boolean;
};
...
/** ETA absoluta (lateArrivalEtaAt o inicio + minutos, tope T+60). */
...
/** Umbral posible ausencia / ETA vencida (igual que detectarAusencias). */
...
export function computeOpsLateArrivalMonitorState(
  ...
  const isLateNotified =
    hasLate && !isPotentialAbsence;
...
/** Texto badge CC para tarjeta / panel / supervisión. */
