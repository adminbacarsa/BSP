...
export default function EmployeesPage() {
  ...
  const [activeTab, setActiveTab] = useState<'legajos' | 'ausencias' | 'feriados' | 'convenios' | 'correcciones'>('legajos');
  ...
  return (
    <DashboardLayout>
        ...
        <div className="max-w-full mx-auto space-y-4 animate-in fade-in h-[calc(100vh-100px)] flex flex-col print:hidden">
            ...
            {activeTab === 'ausencias' && (<div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border dark:border-slate-700 p-6 overflow-hidden flex flex-col"><div className="flex flex-wrap items-center gap-2 mb-6"><div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900 px-4 py-2 rounded-xl border dark:border-slate-700 flex-1 min-w-[180px] max-w-xs"><Search size={16} className="text-slate-400 shrink-0"/><input placeholder="Buscar empleado..." className="bg-transparent outline-none w-full text-sm font-bold text-slate-900 dark:text-white" value={absenceSearchTerm} onChange={e => setAbsenceSearchTerm(e.target.value)}/></div><select className="bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl px-3 py-2 text-xs font-black uppercase text-slate-700 dark:text-white outline-none" value={absenceTypeFilter} onChange={e => setAbsenceTypeFilter(e.target.value)}><option value="">Todos los tipos</option>{NOVEDAD_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select><select className="bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl px-3 py-2 text-xs font-black uppercase text-slate-700 dark:text-white outline-none" value={absenceStatusFilter} onChange={e => setAbsenceStatusFilter(e.target.value)}><option value="">Todos los estados</option><option value="Confirmada">Confirmada</option><option value="Pendiente">Pendiente</option><option value="En verificación">En verificación</option><option value="Autorizada">Autorizada</option><option value="Justificada">Justificada</option><option value="Injustificada">Injustificada</option><option value="Rechazada">Rechazada</option></select><div className="flex rounded-xl border border-slate-200 dark:border-slate-600 overflow-hidden shrink-0"><button type="button" onClick={() => setAbsenceDateFilterMode('month')} className={`px-3 py-2 text-[10px] font-black uppercase transition-colors ${absenceDateFilterMode === 'month' ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-700 text-slate-600 dark:text-white'}`}>Mes</button><button type="button" onClick={() => setAbsenceDateFilterMode('days')} className={`px-3 py-2 text-[10px] font-black uppercase transition-colors ${absenceDateFilterMode === 'days' ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-700 text-slate-600 dark:text-white'}`}>Días</button></div>{absenceDateFilterMode === 'month' ? (<select className="bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl px-3 py-2 text-xs font-black uppercase text-slate-700 dark:text-white outline-none" value={absencePeriodFilter} onChange={e => setAbsencePeriodFilter(e.target.value)}><option value="">Todos los períodos</option>{absencePeriods.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}</select>) : null}{(absenceTypeFilter || absenceStatusFilter || absencePeriodFilter || absenceDateFilterMode === 'days' || absenceSelectedDays.size > 0) && <button onClick={() => { setAbsenceTypeFilter(''); setAbsenceStatusFilter(''); const n = new Date(); setAbsencePeriodFilter(`${n.getFullYear()}-${String(n.getMonth() + 1).padStart(2, '0')}`); setAbsenceDateFilterMode('month'); setAbsenceSelectedDays(new Set()); }} className="px-3 py-2 rounded-xl text-xs font-black uppercase text-slate-400 hover:text-rose-500 border border-slate-200 dark:border-slate-600 hover:border-rose-300 transition-colors">✕ Limpiar</button>}</div>{absenceDateFilterMode === 'days' && (<div className="mb-4 p-4 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900/50"><div className="flex items-center justify-between mb-3"><button type="button" onClick={() => { const [y, m] = absenceCalendarMonth.split('-').map(Number); const d = new Date(y, m - 2, 1); setAbsenceCalendarMonth(`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`); }} className="p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700"><ChevronLeft size={16}/></button><span className="text-xs font-black uppercase text-slate-600 dark:text-slate-300">{absencePeriods.find(p => p.value === absenceCalendarMonth)?.label || absenceCalendarMonth}</span><button type="button" onClick={() => { const [y, m] = absenceCalendarMonth.split('-').map(Number); const d = new Date(y, m, 1); setAbsenceCalendarMonth(`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`); }} className="p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700"><ChevronRight size={16}/></button><button type="button" onClick={() => setAbsenceSelectedDays(new Set(absenceCalendarCells.filter(Boolean) as string[]))} className="text-[10px] font-bold text-indigo-600 hover:underline ml-2">Todo el mes</button><button type="button" onClick={() => setAbsenceSelectedDays(new Set())} className="text-[10px] font-bold text-slate-400 hover:underline ml-2">Limpiar días</button></div><div className="grid grid-cols-7 gap-1 text-center"><div className="text-[9px] font-black text-slate-400 py-1">D</div><div className="text-[9px] font-black text-slate-400 py-1">L</div><div className="text-[9px] font-black text-slate-400 py-1">M</div><div className="text-[9px] font-black text-slate-400 py-1">M</div><div className="text-[9px] font-black text-slate-400 py-1">J</div><div className="text-[9px] font-black text-slate-400 py-1">V</div><div className="text-[9px] font-black text-slate-400 py-1">S</div>{absenceCalendarCells.map((day, i) => day ? (<button key={day} type="button" onClick={() => toggleAbsenceCalendarDay(day)} className={`py-1.5 rounded-lg text-[11px] font-bold font-mono transition-colors ${absenceSelectedDays.has(day) ? 'bg-rose-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-rose-50 dark:hover:bg-rose-900/20'}`}>{parseInt(day.split('-')[2], 10)}</button>) : (<div key={`pad-${i}`} className="py-1.5"/>))}</div><p className="text-[10px] text-slate-400 mt-2 font-bold">{absenceSelectedDays.size > 0 ? `${absenceSelectedDays.size} día(s) seleccionado(s) — novedades que incluyen esas fechas` : `Mostrando todo ${absenceCalendarMonth} — marcá días para acotar`}</p></div>)}<div className="flex-1 overflow-auto custom-scrollbar"><table className="w-full text-left"><thead className="bg-slate-50 dark:bg-slate-900 sticky top-0"><tr>{canAdjust && <th className="p-3 w-10"><input type="checkbox" style={{width:16,height:16,display:'block',cursor:'pointer',accentColor:'#e11d48'}} checked={filteredAbsences.length > 0 && filteredAbsences.every(a => selectedAbsenceIds.has(a.id!))} onChange={e => { const ids = filteredAbsences.map(a => a.id!); setSelectedAbsenceIds(e.target.checked ? new Set(ids) : new Set()); }}/></th>}<th className="p-4 text-[10px] font-black uppercase text-slate-400">Empleado</th><th className="p-4 text-[10px] font-black uppercase text-slate-400">Tipo / Motivo</th><th className="p-4 text-[10px] font-black uppercase text-slate-400">Periodo</th><th className="p-4 text-[10px] font-black uppercase text-slate-400 text-center">Estado</th><th className="p-4 text-[10px] font-black uppercase text-slate-400 text-center">Certificado</th><th className="p-4 text-[10px] font-black uppercase text-slate-400 text-center">Cobertura</th><th className="p-4 text-right"></th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-700">{filteredAbsences.map(a => { const rowBg = selectedAbsenceIds.has(a.id!) ? 'bg-rose-50 dark:bg-rose-900/10' : a.type === 'No Presentación' && a.status === 'Confirmada' ? 'bg-blue-50/40 dark:bg-blue-900/10' : a.status === 'Injustificada' ? 'bg-rose-50/30 dark:bg-rose-900/10' : a.status === 'Justificada' ? 'bg-emerald-50/30' : ''; return (<tr key={a.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${rowBg}`}>{canAdjust && <td className="p-3 w-10"><input type="checkbox" style={{width:16,height:16,display:'block',cursor:'pointer',accentColor:'#e11d48'}} checked={selectedAbsenceIds.has(a.id!)} onChange={e => { setSelectedAbsenceIds(prev => { const next = new Set(prev); e.target.checked ? next.add(a.id!) : next.delete(a.id!); return next; }); }}/></td>}<td className="p-4 font-bold text-sm text-slate-900 dark:text-white uppercase">{getAbsenceEmployeeName(a)}</td><td className="p-4"><div className="flex flex-col"><span className="text-xs font-bold uppercase">{a.type}</span><span className="text-[10px] text-slate-500">{a.reason || '-'}</span></div></td><td className="p-4 text-xs font-mono text-slate-500">{getArgentinaDate(a.startDate)} - {getArgentinaDate(a.endDate)}</td><td className="p-4 text-center">{renderAbsenceStatusCell(a)}</td><td className="p-4 text-center">{a.hasCertificate ? <span className="text-emerald-500 flex justify-center"><FileCheck size={16}/></span> : <span className="text-slate-300">-</span>}</td><td className="p-4 text-center"><span className={`px-2 py-1 rounded text-[10px] font-black uppercase ${coberturaBadgeClass(a.coberturaEstado)}`}>{a.coberturaEstado || 'PENDIENTE'}</span>{(!a.coberturaEstado || a.coberturaEstado === 'PENDIENTE') && a.status !== 'Rechazada' && <p className="text-[9px] text-slate-400 mt-1 font-bold">Gestionar en Planificación</p>}</td><td className="p-4 text-right flex justify-end gap-2">{(a.status === 'Pendiente' || a.status === 'En verificación') && <button title="Rechazar" onClick={() => handleOpenAbsenceModal({...a, status: 'Rechazada'})} className="text-slate-400 hover:text-red-600 text-[10px] font-black uppercase px-2 py-1 rounded hover:bg-red-50 transition-colors">✕</button>}<button onClick={() => handleOpenAbsenceModal(a)} className="text-slate-400 hover:text-indigo-500"><Edit2 size={16}/></button><button onClick={() => handleDeleteAbsence(a.id!)} className="text-slate-400 hover:text-rose-500"><Trash2 size={16}/></button></td></tr>); })}</tbody></table></div></div>)}
        </div>
