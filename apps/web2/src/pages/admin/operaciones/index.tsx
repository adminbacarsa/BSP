...
const CoverageModalContent = ({ isOpen, onClose, absenceShift, logic, opsCaps }: any) => {
    ...
    const markCoverageResolved = (batch: ReturnType<typeof writeBatch>, coverageType: string, coveringEmployee?: any, covererShiftId?: string | null) => {
        if (!absenceShift?.id || absenceShift.isVirtual || String(absenceShift.id).startsWith('V124_') || String(absenceShift.id).startsWith('SLA_GAP')) return null;
        if (!(absenceShift.isUnassigned || absenceShift.isAbsent || absenceShift.isPotentialAbsence)) return null;
        ...
        const isAbsence = !!(absenceShift.isAbsent || absenceShift.isPotentialAbsence || titular.titularShiftId);
        return applyCoverageLedgerToBatch(batch, {
            vacancyShiftId: titular.vacancyShiftId,
    ...
    const handleRetener = async (s: any) => {
        ...
        try {
            ...
            markCoverageResolved(batch, 'RETENTION', s, s.id);
            await batch.commit();
            await Promise.race([
    ...
    const handleAdelantar = async (s: any) => {
        ...
        try {
            ...
            markCoverageResolved(batch, 'EARLY_START', s, s.id);
            await batch.commit();
            await Promise.race([
                waitForPendingWrites(db),
                new Promise<void>((_, reject) => setTimeout(() => reject(new Error('sync_timeout')), 8000)),
    ...
    const handleFranco = async (s: any) => {
        ...
        try {
            ...
            markCoverageResolved(batch, 'FRANCO', s, s.id);
            await batch.commit();
            await Promise.race([
                waitForPendingWrites(db),
                new Promise<void>((_, reject) => setTimeout(() => reject(new Error('sync_timeout')), 8000)),
...
export default function OperacionesPage() {
    ...
    const handleMarkAbsent = async (shift: any) => {
        try {
            ...
            // Marcar turno ausente + anclar absenceId (vacante la crea onGuardAbsenceDetected)
            ...
            // Notificar a RRHH y Planificación vía novedades
            await addDoc(collection(db, 'novedades'), stampEmpresaId({
    ...
    const handleRevertAbsence = async (shift: any) => {
        if (!confirm(`¿Revertir la ausencia de ${shift.employeeName}?\nSe limpiará el flag, la vacante sin cubrir y el registro RRHH. Usá esto solo si fue un error.`)) return;
        try {
            const result = await revertOpsAbsence({
                shiftId: shift.id,
    ...
    const handleDeclareAbsentT5 = async (shift: any) => {
        ...
        await addDoc(collection(db, 'novedades'), stampEmpresaId({
            ...
            shiftId: shift.id, absenceId, objectiveName: shift.objectiveName || '',
            positionName: shift.positionName || '',
