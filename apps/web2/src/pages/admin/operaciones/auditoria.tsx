...
const AdvancedResolutionModal = ({ isOpen, onClose, type, data, onConfirm, logic }: any) => {
    ...
    const shiftStartStr = data.shift.shiftDateObj ? toDate(data.shift.shiftDateObj).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}) : '';
    const objName = data.shift.objectiveName || '';
    const openWhatsApp = (target: any, ctx?: string) => {
        const ph = target.phone || target.celular || '';
        const nombre = target.fullName || target.employeeName || '';
        if (!ph) { alert('Sin teléfono registrado'); return; }
        let msg = '';
        if (ctx === 'INTERCAMBIO') msg = waMensaje.intercambio(nombre, objName);
        else if (isAbsence || ctx === 'COBERTURA' || ctx === 'ADELANTO') msg = waMensaje.cobertura(nombre, objName, shiftStartStr, shiftEndStr);
        else msg = waMensaje.bienvenida(nombre);
        waOpen(ph, msg);
    };
