...
export default function PlanificacionPage() {
    ...
    const isEmployeeOnSelectedObjective = useCallback((e: { id: string; preferredObjectiveId?: string }) => {
        ...
        // En modo grupo: incluir empleados de cualquiera de los objetivos del grupo
        if (selectedGrupo) {
        ...
        return false;
    }, [selectedObjective, slaIdToObjId, activeGuestIdsForObjective, selectedGrupo]);
    ...
    const handleContextChange = (newClient: string, newObjective: string) => { if (Object.keys(pendingChanges).length > 0) { if (!confirm(`⚠️ TIENES CAMBIOS SIN GUARDAR.\n¿Descartar y cambiar de objetivo?`)) return; setPendingChanges({}); setPendingNovedades({}); } setSelectedGrupo(null); setSelectedClient(newClient); setSelectedObjective(newObjective); setSearchTerm(''); setBandFilter(null); setSelection({start: null, end: null}); setComparingSnapshot(null); setOpenDrop(null); setAutoGeneratedReady(false); };
                        ...
                        <>
                            ...
                            <div className="flex items-center gap-1.5 no-print">
                                {selectedGrupo ? (
                                    ...
                                    <>
                                        <span className="flex items-center gap-1.5 bg-violet-700 text-white px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wide cursor-default select-none">
