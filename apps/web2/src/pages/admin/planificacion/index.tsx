...
import { resolveCellSecondBlock } from '@/lib/planificacion/splitShiftDisplay';
import {
...
import PlanningVacancyModal from '@/components/planificacion/PlanningVacancyModal';
            ...
            <thead className="sticky top-0 z-30 bg-slate-100 shadow-md">
                ...
                <tr className={hasSlaExcludedDatesInMonth ? 'h-11' : 'h-10'}>
                    {daysInMonth.map((d, dayIndex) => {
                        ...
                        const isHeaderPastPublished = (dateStr < getDateKey(new Date())) && isCronogramaPublicado && !correctionMode;
                        return (
                            <th
                                key={d.toISOString()}
                                ...
                                className={`min-w-[25px] border-b-2 border-r p-0.5 text-center select-none cursor-pointer transition-colors
                                    ${isSource ? 'bg-indigo-600 text-white' : isInSel && columnSelectMode ? 'bg-indigo-100 dark:bg-indigo-900/40' : hasExclusion ? 'bg-rose-100/90 dark:bg-rose-950/40 border-b-rose-300 dark:border-rose-800' : isHeaderPastPublished ? 'bg-slate-200 dark:bg-slate-700 border-b-slate-300 dark:border-slate-600' : isWeekend ? 'bg-rose-50 dark:bg-rose-900/30' : 'hover:bg-slate-100 dark:hover:bg-slate-700 dark:border-slate-700 border-b-slate-200'}`}
                            ...
                            {!isSnapshotView && (
                                ...
                                >
                                    ...
                                    {daysInMonth.map((day, dayIndex) => {
                                        ...
                                        const isPastAndPublished = isCronogramaPublicado && !correctionMode && isPastDay;
                                        if (s && !isSnapshotView) {
            ...
            <div className={`flex flex-col animate-in fade-in select-none transition-all duration-300 ease-in-out min-h-0 ${cronoFullscreen ? 'fixed inset-0 z-[1100] bg-white dark:bg-slate-900 overflow-hidden p-1 space-y-1' : comparingSnapshot && selectedObjective ? 'h-[calc(100dvh-3.75rem)] overflow-hidden p-0.5 space-y-0.5' : selectedClient ? 'h-[calc(100dvh-5.5rem)] lg:h-[calc(100dvh-6.5rem)] overflow-hidden p-1 space-y-1.5' : 'p-2 space-y-4 h-[calc(100vh-220px)] lg:h-[calc(100vh-160px)]'}`} onMouseUp={handleMouseUp} onClick={() => setEmpPosPicker(null)}>
                ...
                {showVacancyModal && (
                    <PlanningVacancyModal
                        ...
                        vacancyActiveDates={vacancyActiveDates}
                        setVacancyActiveDates={setVacancyActiveDates}
                        employees={employees}
                        currentDate={currentDate}
                        pendingChanges={pendingChanges}
                        shiftsMap={shiftsMap}
                        effectivePosStructure={effectivePosStructure}
                        selectedReplacement={selectedReplacement}
                        setSelectedReplacement={setSelectedReplacement}
                        vacancyDayCoverages={vacancyDayCoverages}
                        setVacancyDayCoverages={setVacancyDayCoverages}
                        vacancyEditingDay={vacancyEditingDay}
                        setVacancyEditingDay={setVacancyEditingDay}
                        setVacancyApplyToAllSelected={setVacancyApplyToAllSelected}
                        vacancyReplacementOpen={vacancyReplacementOpen}
                        setVacancyReplacementOpen={setVacancyReplacementOpen}
                        vacancyPickerTab={vacancyPickerTab}
                        setVacancyPickerTab={setVacancyPickerTab}
                        vacancySplitExtId={vacancySplitExtId}
                        setVacancySplitExtId={setVacancySplitExtId}
                        vacancySplitAdelId={vacancySplitAdelId}
                        setVacancySplitAdelId={setVacancySplitAdelId}
                        vacancySplitExtExtraHours={vacancySplitExtExtraHours}
                        setVacancySplitExtExtraHours={setVacancySplitExtExtraHours}
                        vacancySplitSecondExtraHours={vacancySplitSecondExtraHours}
                        setVacancySplitSecondExtraHours={setVacancySplitSecondExtraHours}
                        activePosition={activePosition}
                        vacancyGapBandOverride={vacancyGapBandOverride}
                        setVacancyGapBandOverride={setVacancyGapBandOverride}
                        vacancyApplyToAllSelected={vacancyApplyToAllSelected}
                        selectedObjectiveData={selectedObjectiveData}
                        selectedObjective={selectedObjective}
                        vacancyReplacementSearch={vacancyReplacementSearch}
                        setVacancyReplacementSearch={setVacancyReplacementSearch}
                        vacancyFrancoAuthApproved={vacancyFrancoAuthApproved}
                        setVacancyFrancoAuthApproved={setVacancyFrancoAuthApproved}
                        requestSupervisorFrancoAuth={requestSupervisorFrancoAuth}
                        vacancyReplacementPanelRef={vacancyReplacementPanelRef}
                        finalizeVacancyModal={finalizeVacancyModal}
                        handleProcessVacancy={handleProcessVacancy}
                    />
                )}
