...
export function assertAfipCertCurrentlyValid(certPem: string): void {
  ...
  if (Number.isFinite(from) && now < from) {
    throw new Error(
      `Certificado AFIP aún no vigente (válido desde ${x.validFrom}). Esperá a esa fecha o subí el certificado activo en AFIP.`,
    );
  }
  if (Number.isFinite(to) && now > to) {
    throw new Error(
      `Certificado AFIP vencido (${x.validTo}). Generá uno nuevo en AFIP y ejecutá npm run afip:secrets.`,
    );
  }
}

export function formatAfipCallError(e: unknown, production: boolean): string {
  const err = e as {
    message?: string;
    response?: { status?: number; data?: string | Record<string, unknown> };
  };
  ...
  if (status === 401 || /\b401\b/.test(msg)) {
    if (/afipsdk|access_token/i.test(msg)) {
      return (
        'El SDK anterior requería access_token de afipsdk.com. Ya usamos WSAA directo; ' +
        'redeploy de functions:lookupClientByCuit y probá de nuevo.'
      );
    }
    const env = production ? 'producción' : 'homologación';
    return (
      `AFIP rechazó el certificado (401, entorno ${env}). Revisá: (1) certificado vigente y clave del mismo CSR, ` +
