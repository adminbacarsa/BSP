...
export const onNovedadCreated = functions
  ...
  .onCreate(async (snap) => {
    ...
    // Buscar sesión de operador activa para esta empresa
    ...
    // Obtener tokens FCM: si hay operador activo → solo su token; sino → todos los tokens de la empresa
    let tokensQuery: admin.firestore.Query;
    if (operatorId) {
      tokensQuery = db.collection('device_tokens').where('uid', '==', operatorId);
    } else {
      // Sin operador: notificar a todos los device_tokens (admins/supervisores)
