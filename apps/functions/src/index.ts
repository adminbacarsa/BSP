...
export const gestionarVacantes = functions
  ...
  .onRun(async () => {
    ...
    for (const docSnap of snap.docs) {
      ...
      // ── T-1h o ya iniciado: Protocolo + cascada Auto ─────────────────
      if ((minutesUntil < 0 || minutesUntil <= 60) && !shift.vacanteCascadaAt) {
        ...
        // Si estaba "devuelta" solo a plan, reabrir para Ops/cascada en ventana urgente
      ...
      // ── T-2h: alerta Ops (sin cascada aún) ───────────────────────────
      // Cambio de última hora (<2h) también entra acá.
      ...
      // ── > T-2h: asegurar novedad a Planificación (sin quitar CUBRIR a Ops) ─
      if (minutesUntil > 120 && !shift.vacanteReportadaAt) {
        await docSnap.ref.update({
          vacanteReportadaAt: now,
          // No marcar isReportedToPlanning: Ops debe poder cubrir mientras Plan asigna.
        });
