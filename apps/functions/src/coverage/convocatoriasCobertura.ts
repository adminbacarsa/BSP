...
async function resolverCobertura(
  ...
  if (conv.type === 'EXTEND' && conv.extendShiftId) {
    ...
    if (conv.ftShiftId) {
      ...
      batch.update(ftRef, {
        code: String(conv.shiftCode || (vacantData as any).code || 'M').toUpperCase(),
        isFranco: false,
        ...
        coversPositionName: (vacantData as any).positionName || conv.positionName || null,
        coversBandCode: String(conv.shiftCode || (vacantData as any).code || 'M').toUpperCase(),
        ...
        empresaId: conv.empresaId || (vacantData as any).empresaId || null,
        ...demoCovererPresenceFields(resolvedBy, gapPresentAt),
        ...covererLedgerFields({
          ...ledgerBase,
          vacancyShiftId: titular.vacancyShiftId || conv.shiftId,
