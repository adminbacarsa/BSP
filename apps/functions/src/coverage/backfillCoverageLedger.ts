/**
 * Backfill P2: asigna coverageEventId a coberturas históricas del día
 * que ya tienen señales (causedBy / covers / coveredBy) pero sin ledger.
 */
import * as functions from 'firebase-functions/v1';
import * as admin from 'firebase-admin';
import { newCoverageEventId } from './coverageLedger';

const SUPER_ADMIN_ROLES = ['SuperAdmin', 'SUPERADMIN', 'SUPER_ADMIN', 'SP'];

type ShiftRow = { id: string; data: Record<string, any> };

function isVacancyLike(d: Record<string, any>): boolean {
...
function buildMatches(rows: ShiftRow[]): Match[] {
  const byId = new Map(rows.map((r) => [r.id, r]));
  const used = new Set<string>();
  const matches: Match[] = [];

  const claim = (...ids: Array<string | null | undefined>) => {
    for (const id of ids) {
      if (id) used.add(id);
    }
  };

  // 1) Vacante COVERED con causedBy + coveredBy
  for (const row of rows) {
    if (used.has(row.id)) continue;
    if (String(row.data.coverageEventId || '').trim()) continue;
    if (!isVacancyLike(row.data)) continue;
    ...
    if (!coveredById && !coveredByName && row.data.status !== 'COVERED') continue;

    const titularId =
      (row.data.causedByShiftId && byId.has(String(row.data.causedByShiftId))
    ...
    let covererId: string | null = null;
    if (coveredById) {
      const hit = rows.find(
        (r) =>
          !used.has(r.id)
      ...
      if (hit) covererId = hit.id;
    }

    const eventId = newCoverageEventId();
    const titularName = String(
      row.data.causedByEmployeeName
  ...
  // 2) Cubridor con covers* sin eventId
  for (const row of rows) {
    if (used.has(row.id)) continue;
    if (String(row.data.coverageEventId || '').trim()) continue;
    if (isVacancyLike(row.data)) continue;
    ...
    if (!coversEmpId && !coversName) continue;
    ...
    if (row.data.causedByShiftId && byId.has(String(row.data.causedByShiftId))) {
      ...
      if (hit) titularId = hit.id;
    } else if (coversName.length > 2) {
      const hit = rows.find((r) => {
        if (used.has(r.id) || isVacancyLike(r.data)) return false;
  ...
  // 3) Titular ausente con coveredBy sin eventId
  for (const row of rows) {
    if (used.has(row.id)) continue;
    if (String(row.data.coverageEventId || '').trim()) continue;
    if (isVacancyLike(row.data)) continue;
    if (!(row.data.isAbsent === true || String(row.data.status || '').toUpperCase() === 'ABSENT')) continue;
    const coveredById = row.data.coveredByEmployeeId ? String(row.data.coveredByEmployeeId) : '';
    const coveredByName = String(row.data.coveredByEmployeeName || '').trim();
    if (!coveredById && !coveredByName) continue;
      ...
      rows.find(
        (r) =>
          ...
          && (String(r.data.causedByShiftId || '') === row.id
            || String(r.data.originRef || '') === row.id),
      )?.id || null;
...
export const backfillCoverageLedger = functions
  .runWith({ timeoutSeconds: 120, memory: '512MB' as const })
  .https.onCall(async (data: { empresaId?: string; dateStr?: string; dryRun?: boolean }, context) => {
    if (!context.auth?.uid) {
