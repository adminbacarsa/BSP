...
export const ASSISTANT_FUNCTION_DECLARATIONS = [
  ...
  },
  {
    ...
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        texto_cliente: { type: SchemaType.STRING, description: 'Fragmento del nombre del cliente en CRM.' },
        limite: { type: SchemaType.NUMBER, description: 'Máximo de objetivos a listar (default 40, máx 60).' },
      },
  ...
  },
  {
    ...
    description:
      'Cuenta legajos de la empresa: cuenta_para_tarjeta_panel_empleados_nomina (misma regla que tarjeta EMPLEADOS EN NÓMINA del panel) y criterio amplio RRHH. Usar para «cuántos empleados en nómina/plantilla».',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        fecha_referencia: { type: SchemaType.STRING, description: 'YYYY-MM-DD opcional para rotular el mes.' },
      },
      required: [],
    },
  },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        limite: { type: SchemaType.NUMBER, description: 'Máximo de filas (8–160, default 120).' },
      },
      required: [],
    },
  },
  ...
  },
  {
    ...
    description:
      'Para **un** objetivo: «cuántas horas a planificar», horas vendidas SLA vs planificadas en un mes. Si son **varios** contratos listados en el chat, usá **resumen_horas_sla_varios_objetivos** en su lugar. fecha_referencia = cualquier día del mes (ej. junio → 2026-06-15). texto_objetivo = nombre del sitio.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        id_objetivo: {
  ...
  },
  {
    ...
    description:
      'Totales de liquidación operativa de la empresa en un mes o rango (misma lógica que Reportes): hs_reales fichadas, diurnas, nocturnas, al_100_ft (FT), al_50 (bolsa >200h), plus_feriado, bolsa_200, hs_simples. Usar para «horas extras en mayo», «horas al 100%», «diurnas y nocturnas», «cantidad para liquidar», «FT trabajados». fecha_referencia = cualquier día del mes; o fecha_desde/fecha_hasta.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        fecha_desde: { type: SchemaType.STRING, description: 'YYYY-MM-DD inicio inclusive.' },
        fecha_hasta: { type: SchemaType.STRING, description: 'YYYY-MM-DD fin inclusive.' },
        limite: { type: SchemaType.NUMBER, description: 'Máximo de filas en la muestra (default 40, máx 80).' },
      },
      required: [],
    },
  },
...
ASSISTANT_FUNCTION_DECLARATIONS.push(
  ...
  } as any,
  {
    ...
    description:
      'Marca automáticamente presencia en los turnos que ya iniciaron sin fichar, y cierra los turnos que ya terminaron y siguen activos. Si hay un relevo pendiente (guardia que viene a reemplazar pero aún no llegó), el turno saliente queda en retención en vez de cerrarse. Ideal para «activá el modo demo», «dar presentes automáticos», «cerrar turnos que terminaron», «activá auto presencia». Con simulacion=true solo muestra qué haría sin modificar nada.',
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        simulacion: {
