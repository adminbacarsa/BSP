/**
 * VPLAN — cómo planificar (metodología antes de asignar guardias).
 * Complementa planningTarget (QUÉ) con reglas de CÓMO.
 */

import { COVERAGE_LADDER } from './vplan.brain-model';
import { getRotationProfile } from './vplan.rotation';
import { is24hsPosition, type VplanPositionDef } from './vplan.positions';
import type {
  VplanCycleSemantics,
  VplanDemandModel,
  VplanFeasibilityReport,
  VplanPlanningMethod,
  VplanPlanningMethodMandate,
  VplanPlanningMethodPipelineStep,
  VplanPlanningMethodPositionRule,
  VplanRunMode,
  VplanStrategy,
  VplanSupplyModel,
} from './vplan.types';

function buildMandates(cycleSemantics?: VplanCycleSemantics): VplanPlanningMethodMandate[] {
...
function buildPipelineSteps(mode: VplanRunMode, engine: string): VplanPlanningMethodPipelineStep[] {
  return [
    {
      step: 1,
      phase: '4_strategy',
...
function buildPositionAssignmentRules(
...
export function buildVplanPlanningMethod(opts: {
  ...
  const { strategy } = opts;
  const rot = getRotationProfile(strategy.cycle);
  ...
  const coverageLadder = COVERAGE_LADDER.map((step) => {
