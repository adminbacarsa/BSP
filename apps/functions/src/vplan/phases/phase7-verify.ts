...
import { detectCrossMonthContinuityViolations,
  detectIllegalBandTransitions,
  restHoursBetweenShiftAssignments,
  workBand,
...
export function runVplanVerification(opts: {
  ...
  const nrMarkers = opts.draft.assignments.filter(
    (a) => String(a.code || '').toUpperCase() === 'NR',
  );
  ...
  const bandViolations = detectIllegalBandTransitions(
    opts.draft,
    dateStrs,
    rules.minRestHoursBetweenBands,
  );
  for (const v of bandViolations) {
    const prevBand = workBand(v.fromCode);
    const nextBand = workBand(v.toCode);
    const restDetail = prevBand && nextBand
      ? ` · ${Math.round(restHoursBetweenShiftAssignments(v.fromDate, prevBand, v.toDate, nextBand) * 10) / 10}h descanso`
      : '';
    issues.push({
      severity: 'blocking',
  ...
  if (opts.strategy.modes.useTrailing && prevMonthLastDate && monthFirstDate) {
    const crossMonth = detectCrossMonthContinuityViolations({
      draft: opts.draft,
  ...
  const maxStreak = workDaysForCycle(opts.strategy.cycle, rules);
  const cctStreakViolations = detectCctStreakViolations({
    draft: opts.draft,
    dateStrs,
    cycle: opts.strategy.cycle,
    previousMonthAssignments: opts.snapshot.previousMonthAssignments,
  ...
  for (const v of cctStreakViolations) {
    // ×(máx+1) = un franco trabajado típico → warning; ×(máx+2)+ → bloqueante
  ...
  const consecutiveHoursViolations = detectConsecutiveBillableHoursViolations(
    opts.draft,
    dateStrs,
    rules.maxConsecutiveWorkHours,
  );
  ...
  let coverageAudit = opts.demand
    ? buildDetailedCoverageAudit({
        draft: opts.draft,
        demand: opts.demand,
        positions: positionsForCycle(opts.snapshot.positions, opts.strategy.cycle),
        defaultPositionByEmp: mergedDefaultPositionByEmp,
        dateStrs,
        cycle: opts.strategy.cycle,
        previousMonthAssignments: opts.snapshot.previousMonthAssignments,
  ...
  const fixedBandViolations = detectFixedBandViolations(
    opts.draft,
    dateStrs,
    mergedDefaultShiftByEmp,
    mergedDefaultPositionByEmp,
  );
  ...
  const positionViolations = detectAssignedPositionViolations(
    opts.draft,
    mergedDefaultPositionByEmp,
    opts.snapshot.positions,
  );
