/**
 * VPLAN — enforcement genérico de qty SLA por objetivo/puesto/banda.
 * Aplica a custom (EN/RO), 24hs (M/T/N), 12hs (D12/N12) y cualquier puesto del SLA.
 */
...
export function isVacanteId(empId: string): boolean {
  return isVirtualEmployeeId(empId);
}

export function normBandCode(code: string): string {
...
/** Dotación máxima de guardias asignables a un puesto según SLA y ciclo. */
export function maxEmployeesForPosition(pos: VplanPositionDef, cycle?: string): number {
  const qty = Math.max(1, Number(pos.qty) || 1);
  if (is24hsPosition(pos)) return qty * headcountPerQtyUnit(cycle);
  return qty;
}

/** Prioridad al recortar: titular real > defaultPosition > VACANTE. */
export function prioritizeEmployeeIds(
  empIds: string[],
  defaultPositionByEmp: Record<string, string>,
  posName: string,
): string[] {
  return [...empIds].sort((a, b) => {
    if (isVacanteId(a) && !isVacanteId(b)) return 1;
    if (!isVacanteId(a) && isVacanteId(b)) return -1;
...
/**
 * Recorta defaultPositionByEmp al cupo SLA del puesto.
 * Evita VACANTE + titular en custom qty=1, o 5+ guardias en 24hs qty=1 (máx 4).
 */
export function capDefaultPositionByEmp(
  ...
  for (const [empId, posName] of Object.entries(map)) {
    ...
    if (!name || !posByName.has(name) || isVirtualEmployeeId(name)) continue;
...
function shiftAppliesOnDay(
  shift: { code?: string; days?: string[] },
  dayLetter: string,
): boolean {
  const c = String(shift.code || '').toUpperCase();
  if (!c || NON_BILLABLE.has(c) || ABSENCE_CODES.has(c)) return false;
...
function assignmentPriority(
  ...
  if (protectedCells?.has(key)) return 0;
  if (isVacanteId(a.employeeId)) return 3;
  const posName = resolvePosName(a, defaultPos);
  if (posName && defaultPos[a.employeeId] === posName) return 1;
  return 2;
}

/**
 * Resuelve qué empleados ocupan cada puesto respetando cupo SLA.
 * Usado por custom EN/RO y validación de plantilla.
 */
export function resolvePositionAssignees(opts: {
  defaultPositionByEmp: Record<string, string>;
  positions: VplanPositionDef[];
  cycle?: string;
  ...
  const posList = opts.onlyCustom
    ? opts.positions.filter((p) => isCustomFixedShiftPosition(p))
    : opts.positions;
  ...
  const empToPos = new Map<string, string>(Object.entries(capped));

  if (opts.draftAssignments) {
    for (const a of opts.draftAssignments) {
      if (empToPos.has(a.employeeId)) continue;
      const code = a.code.toUpperCase();
      for (const pos of posList) {
        if (!posNames.has(pos.positionName)) continue;
        const expected = primaryShiftCode(pos);
        if (code === expected || (is24hsPosition(pos) && normBandCode(code) === normBandCode(expected))) {
          const byPos = new Map<string, string[]>();
          ...
          if (current.length >= maxEmployeesForPosition(pos, opts.cycle)) continue;
...
/**
 * Elimina asignaciones por encima de qty SLA (día × puesto × banda).
 * Custom qty=1: excedente → F.
 * 24hs/12hs: excedente → rebanda a hueco libre del mismo día (no F ciego).
 */
export function stripExcessSlaAssignments(opts: {
  ...
  for (const a of sorted) {
    ...
    if (used >= limit) {
      ...
      if (isCustomFixedShiftPosition(pos)) {
        log.push({
          code: 'SLA_QTY_EXCESS',
      ...
      const rebandOptions = openShiftCodesOnDay(
        pos,
        a.dateStr,
        dayMeta.dayLetter,
        posName,
      ...
      if (rebandOptions.length > 0) {
        ...
        kept.push({
          ...
          hours: shift ? shiftBandHours(shift) : (a.hours ?? 8),
        });
        continue;
      }
...
/**
 * Rellena slots SLA sin cubrir usando guardias en F del mismo puesto asignado.
 * Respeta CCT 6+2 y celdas protegidas de continuidad.
 */
export function fillCoverageGaps(opts: {
  draft: VplanScheduleDraft;
  dateStrs: Array<{ dateStr: string; dayLetter: string }>;
  positions: VplanPositionDef[];
  defaultPositionByEmp: Record<string, string>;
  ...
  function candidateCanFill(empId: string, dateStr: string, shiftCode: string): boolean {
    if (isVirtualEmployeeId(empId)) return false;
    if (opts.protectedCells?.has(protectedCellKey(empId, dateStr))) return false;
    ...
    for (const pos of opts.positions) {
      ...
      for (const shift of shiftsForCycle(pos, opts.cycle)) {
        ...
        while (used < limit) {
          ...
          const pick = candidateIndices.find(({ a }) =>
            candidateCanFill(a.employeeId, dateStr, shiftCode),
          );
          if (!pick) break;
