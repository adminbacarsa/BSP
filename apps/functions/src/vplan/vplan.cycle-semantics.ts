/**
 * VPLAN — semántica del ciclo de trabajo (cerebro).
 *
 * INNEGOCIABLE:
 *   - Mínimo 12h de descanso entre el fin de un turno y el inicio del siguiente.
 *
 * CICLO 6+2 (y afines) NO es "6 días calendario + 2 días calendario":
 *   - Es 6 TURNOS de trabajo + 2 FRANCOS.
 *   - Franco (F) = 24 horas de descanso (no "un día en la grilla" abstracto).
 *   - Ejemplo 6+2: T T T T T T F F  (T = cualquier turno M/T/N/D12/N12).
 *
 * Cobertura SLA (slots por puesto×banda×día) es una capa DISTINTA del ciclo individual.
 */
...
/** Ley inviolable — prioridad 0 (antes de cobertura y horas SLA). */
export const VPLAN_MIN_REST_HOURS_BETWEEN_TURNS = 12;

/** Duración operativa de un Franco planificado. */
export const VPLAN_FRANCO_REST_HOURS = 24;

/** Bloque CCT estándar antes de 2 francos (horas de trabajo acumuladas). */
export const VPLAN_WORK_BLOCK_HOURS_STANDARD = 48;

/** Estiramiento operativo máximo del bloque (contingencia / extensión). */
export const VPLAN_WORK_BLOCK_HOURS_STRETCH = 56;

/** Horas por código de turno (cerebro). Custom EN/RO usan horas SLA propias. */
export const VPLAN_SHIFT_HOURS_BY_CODE: Record<string, number> = {
  M: 8,
  T: 8,
  N: 8,
  D12: 12,
...
const CYCLE_PATTERN_EXAMPLES: Record<string, string> = {
  '6+2': 'M M M M M M F F',
...
/** Patrones de bloque con aritmética de horas explícita. */
const CYCLE_BLOCK_PATTERNS: Array<{
  id: string;
  cycleKey: string;
  label: string;
  pattern: string;
...
export function shiftHoursForTurnCode(code: string): number {
  const c = String(code || '').toUpperCase();
  return VPLAN_SHIFT_HOURS_BY_CODE[c] ?? 0;
}

export function isEightHourTurnCode(code: string): boolean {
  return EIGHT_HOUR_TURNS.has(String(code || '').toUpperCase());
}

export function isTwelveHourTurnCode(code: string): boolean {
  return TWELVE_HOUR_TURNS.has(String(code || '').toUpperCase());
}

export function sumTurnSequenceHours(codes: string[]): number {
  return codes.reduce((s, c) => s + shiftHoursForTurnCode(c), 0);
}

export function isValidWorkBlockHours(totalHours: number, rules?: PlanningRulesConfig | null): {
  ok: boolean;
  level: 'standard' | 'stretch' | 'invalid';
  message: string;
} {
  const resolved = resolvePlanningRules(rules ?? null);
  const standard = VPLAN_WORK_BLOCK_HOURS_STANDARD;
  const stretch = resolved.maxConsecutiveWorkHours ?? VPLAN_WORK_BLOCK_HOURS_STRETCH;
...
export function buildShiftTypeCatalog(): VplanCycleSemantics['shiftTypes'] {
  return [
    {
      group: '8h',
      codes: ['M', 'T', 'N'],
...
export function buildDailyCoverageEquivalence(): VplanCycleSemantics['dailyCoverageEquivalence'] {
  return {
    hoursPerDay: 24,
...
export function buildBlockPatternsForCycle(
  cycleKey: string,
  rules?: PlanningRulesConfig | null,
): VplanCycleSemantics['blockPatterns'] {
  const stretch = resolvePlanningRules(rules ?? null).maxConsecutiveWorkHours
...
export function isBillableWorkTurnCode(code: string, cycle?: string): boolean {
  const c = String(code || '').toUpperCase();
  if (!c || ['F', 'FF', 'FP', 'FT', 'RET', 'R', 'NR'].includes(c)) return false;
  if (['V', 'L', 'E', 'A', 'PG', 'AA'].includes(c)) return false;
  const key = normalizePlanningCycleKey(cycle);
  if (key === '4+2') return c === 'D12' || c === 'N12';
  return ['M', 'T', 'N', 'EN', 'RO', 'RON', 'D12', 'N12', 'ESC', 'REF'].includes(c);
}

export function isFrancoRestCode(code: string): boolean {
  const c = String(code || '').toUpperCase();
  return c === 'F' || c === 'FF' || c === 'FP';
}

/** FT = franco trabajado (24h de descanso interrumpido — costo extra, no cuenta como F de ciclo). */
export function isFrancoTrabajadoCode(code: string): boolean {
  return String(code || '').toUpperCase() === 'FT';
}

export function getVplanCycleDefinition(
  cycle?: string,
  rules?: PlanningRulesConfig | null,
): VplanCycleDefinition {
  const resolved = resolvePlanningRules(rules ?? null);
  const key = normalizePlanningCycleKey(cycle);
  const workTurnCount = workDaysForCycle(key, resolved);
  const restFrancoCount = restDaysForCycle(key, resolved);
  ...
  return {
    ...
    workTurnCount,
    restFrancoCount,
    francoHours: VPLAN_FRANCO_REST_HOURS,
    minRestHoursBetweenTurns: VPLAN_MIN_REST_HOURS_BETWEEN_TURNS,
    patternExample,
    ...
    notCalendarDays: `No son ${workTurnCount} días calendario: son ${workTurnCount} turnos de ${hoursPerTurn}h, `
      + `luego ${restFrancoCount} francos de ${VPLAN_FRANCO_REST_HOURS}h (${hoursFormula} antes del descanso).`,
    shiftHours: resolved.cycles[key]?.shiftHours ?? (key === '4+2' ? 12 : 8),
    workBlockHours,
    hoursFormula,
    standardBlockHours: VPLAN_WORK_BLOCK_HOURS_STANDARD,
    stretchBlockHours: resolved.maxConsecutiveWorkHours ?? VPLAN_WORK_BLOCK_HOURS_STRETCH,
  };
}

export function buildVplanCycleSemantics(
  cycle?: string,
  rules?: PlanningRulesConfig | null,
): VplanCycleSemantics {
...
/** Etiqueta legible del estado de racha de un guardia (turnos, no días calendario). */
export function formatEmployeeTurnStreakLabel(workTurns: number, restFrancos: number): string {
  if (restFrancos > 0) return `Fx${restFrancos} (${restFrancos} franco(s) 24h)`;
  if (workTurns > 0) return `Tx${workTurns} (${workTurns} turno(s) sin F)`;
  return '—';
}
