...
async function runVplanOrchestrator(request) {
    ...
    if (runPhases.has('1_demand')) {
        ...
        steps.push(step('1_demand', true, `${context.demand.planningTarget.summary} · ${context.demand.slaVendidas}h vendidas`, t1));
    }
    ...
    if (runPhases.has('4_strategy')) {
        ...
        context.strategy = (0, phase4_strategy_1.buildVplanStrategy)({
            ...
            positions: snapshot.positions,
            trailingEmployeeCount: (0, vplan_trailing_1.countTrailingEmployees)(snapshot.prevPlanningState),
            planningRules,
        });
        ...
        const semLabel = sem
            ...
            : '';
        steps.push(step('4_strategy', true, `${context.strategy.engine} · ciclo ${context.strategy.cycle}${context.strategy.modes.useTrailing ? ` · racha ${(0, vplan_trailing_1.countTrailingEmployees)(snapshot.prevPlanningState)} guardias` : ''}${semLabel ? ` · ${semLabel}` : ''}`, t4));
    }
    if (runPhases.has('5_generate') && context.strategy) {
        ...
        try {
            ...
            const slotCovLabel = slotCov
                ...
                : '';
            steps.push(step('5_generate', true, `${context.draft.assignments.length} celdas · ${hoursLabel} · ${context.draft.sourceEngine}${slotCovLabel}${context.draft.stats?.trailingSlotCount ? ` · ${context.draft.stats.trailingSlotCount} racha(s)` : ''}${context.draft.stats?.needsReinforcementCount ? ` · ${context.draft.stats.needsReinforcementCount} NR` : ''}${context.draft.stats?.continuityFixes ? ` · ${context.draft.stats.continuityFixes} ajuste(s)` : ''}`, t5));
        }
        catch (e) {
