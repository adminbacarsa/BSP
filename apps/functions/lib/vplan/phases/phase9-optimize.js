...
exports.runVplanOptimization = runVplanOptimization;
const planningGeminiServer_1 = require("../../assistant/planningGeminiServer");
async function runVplanOptimization(opts) {
    if (!opts.enabled) {
        return {
    ...
    for (const emp of opts.supply.employees) {
        if (emp.blockedDates.length)
            ausencias[emp.employeeId] = emp.blockedDates;
    }
    try {
...
//# sourceMappingURL=phase9-optimize.js.map