...
exports.getTaxpayerFromPadron = getTaxpayerFromPadron;
const PADRON_NS = 'http://a5.soap.ws.server.puc.sr/';
function padronUrl(production) {
    return production
...
function buildGetPersonaV2Envelope(creds, cuitRepresentada, idPersona) {
    return [
...
function parseActividadBlock(block) {
    return {
...
function personaNotFoundError(idPersona, production) {
    ...
    return new Error(`CUIT ${cuit} no encontrado en el padrón AFIP (producción). Verificá los 11 dígitos.`);
}
function extractPersonaPayload(soapBody, idPersona, production) {
    ...
    if (errMsgRaw && !hasIdentity) {
        if (isDfePending) {
            throw new Error('AFIP no entrega constancia para este CUIT: tiene pendiente el domicilio fiscal electrónico (RG 4280/18). ' +
            ...
            datosMonotributo: mono
                ? {
                    categoriaMonotributo: pickTag(catMonoBlock, 'descripcionCategoria') || pickTag(mono, 'descripcionCategoria')
                        ? {
                            descripcionCategoria: pickTag(catMonoBlock, 'descripcionCategoria') || pickTag(mono, 'descripcionCategoria'),
                        }
                        : undefined,
                    actividadMonotributista: actMono?.descripcionActividad ? actMono : undefined,
                }
                : undefined,
            datosRegimenGeneral: impuestosRg.length || actividadesRg.length
                ? {
...
function padronHttpError(status, body) {
    const fault = parseSoapFault(body);
    if (fault)
        return `Padrón AFIP: ${fault}`;
...
async function getTaxpayerFromPadron(creds, cuitRepresentada, idPersona, production) {
...
//# sourceMappingURL=padronConstancia.js.map