...
exports.flushShiftNotifDigests = void 0;
exports.buildDigestMessage = buildDigestMessage;
...
async function enqueueShiftNotifDigest(db, params) {
    ...
    const empUid = empDoc.exists ? empDoc.data()?.uid || null : null;
    const empresaId = String(params.empresaId || empDoc.data()?.empresaId || '').trim() || null;
    ...
    await db.runTransaction(async (tx) => {
        ...
        const next = {
            employeeId,
            empresaId: prev.empresaId || empresaId,
            uid: prev.uid || empUid,
...
async function sendDigestPushAndInbox(db, employeeId, data) {
    ...
    try {
        ...
        console.error('[flushShiftNotifDigests] inbox error:', e?.message);
    }
    if (!tokens.length) {
    ...
    try {
        ...
        console.warn('[flushShiftNotifDigests] FCM error:', e?.message);
    }
}
exports.flushShiftNotifDigests = functions
...
//# sourceMappingURL=shiftNotifDigest.js.map