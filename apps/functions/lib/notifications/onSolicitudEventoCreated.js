...
exports.onSolicitudEventoCreated = void 0;
const functions = require("firebase-functions/v1");
const admin = require("firebase-admin");
const firestore_1 = require("firebase-admin/firestore");
function fmtFechaAr(yyyyMmDd) {
    const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(yyyyMmDd);
    if (!m)
        return yyyyMmDd;
    return `${m[3]}/${m[2]}/${m[1]}`;
}
exports.onSolicitudEventoCreated = functions
    ...
    if (!esConvocatoria && !esAsignacionDirecta)
        return;
    ...
    console.log(`[onSolicitudEventoCreated] Notif creada solicitud=${solicitudId} emp=${empleadoId} uid=${empUid || 'null'}`);
});
//# sourceMappingURL=onSolicitudEventoCreated.js.map