...
exports.assignGuardToEventAdmin = assignGuardToEventAdmin;
...
function arDateTimeTs(fecha, hhmm) {
...
function pickExistingTurno(docs) {
    return (docs.find((d) => {
        const c = String(d.data().code || '').toUpperCase();
        return c !== 'EV' && c !== 'F' && c !== 'FF' && c !== 'FP';
    }) ?? null);
}
async function loadTurnosDelDia(db, empleadoId, servicioFecha, empresaId) {
    ...
    for (const d of [...tsSnap.docs, ...strSnap.docs]) {
        if (empresaId && d.data().empresaId && String(d.data().empresaId) !== empresaId)
            continue;
        byId.set(d.id, d);
    }
    return [...byId.values()];
}
async function assignGuardToEventAdmin(db, params) {
    const { empresaId, empleadoId, empleadoNombre, empleadoObjectiveId, empleadoObjectiveName, eventoId, eventoNombre, clienteId, clienteNombre, servicioId, servicioNombre, servicioFecha, horaInicio, horaFin, horas, solicitudId, respondidoPor, } = params;
    ...
    let originalObjectiveId = existingTurno?.data().objectiveId || empleadoObjectiveId || null;
    let originalObjectiveName = existingTurno?.data().objectiveName || empleadoObjectiveName || null;
    ...
    if (endTs.toMillis() <= startTs.toMillis()) {
        endTs = admin.firestore.Timestamp.fromDate(new Date(endTs.toDate().getTime() + 24 * 60 * 60 * 1000));
    }
...
//# sourceMappingURL=eventoAssignAdmin.js.map