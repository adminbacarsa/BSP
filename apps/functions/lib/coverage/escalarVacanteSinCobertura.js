...
exports.escalarVacanteSinCobertura = escalarVacanteSinCobertura;
exports.countColleaguesPresentSameObjective = countColleaguesPresentSameObjective;
exports.loadReemplazarRetiro2a3hFromSla = loadReemplazarRetiro2a3hFromSla;
...
async function notifySupervisorsObjective(db, objectiveId, payload) {
...
async function escalarVacanteSinCobertura(db, params) {
    ...
    if (params.attemptRetention !== false && shift) {
        const ret = await (0, coverageRetention_1.retainOutgoingForGap)(db, { ...shift, id: shiftId });
        if (ret.applied) {
            return {
                escalated: false,
                retained: true,
...
async function countColleaguesPresentSameObjective(db, objectiveId, excludeShiftId, excludeEmployeeId) {
    const oid = String(objectiveId || '').trim();
    if (!oid)
        return 0;
    const snap = await db
    ...
    let n = 0;
    for (const d of snap.docs) {
        if (d.id === excludeShiftId)
            continue;
        const data = d.data();
        if (data.isCompleted === true)
            continue;
        const eid = String(data.employeeId || '').trim();
        if (!eid || eid === excludeEmployeeId || eid === 'VACANTE')
            continue;
        n += 1;
    }
    return n;
}
async function loadReemplazarRetiro2a3hFromSla(db, objectiveId, positionName) {
    const oid = String(objectiveId || '').trim();
    if (!oid)
        return null;
    ...
    for (const d of slaSnap.docs) {
        ...
        for (const p of positions) {
            if (!posMatch(p?.name, positionName))
                continue;
            if (typeof p.reemplazarRetiro2a3h === 'boolean')
                return p.reemplazarRetiro2a3h;
        }
    }
    return null;
}
//# sourceMappingURL=escalarVacanteSinCobertura.js.map