...
exports.rejectSwapRequestSupervisor = exports.approveSwapRequest = exports.cancelSwapRequest = exports.confirmSwapRequest = exports.respondSwapRequest = exports.createSwapRequest = exports.getSwapCandidates = exports.getSwapPeople = void 0;
...
function shiftIsSwappable(shift) {
    ...
    if (['COMPLETED', 'CANCELLED', 'INTERRUPTED', 'ABSENT'].includes(st))
        return false;
    const code = String(shift.code || '').toUpperCase();
    if (FRANCO_CODES.has(code) || shift.isFranco)
        return true;
    return code.length > 0;
}
async function assertPortalEmployee(context) {
    if (!context.auth?.uid) {
...
async function loadShift(db, shiftId) {
...
function mapCandidate(shiftId, data, empName) {
    return {
        shiftId,
...
async function notifySwapParties(db, request, title, body) {
    const empresaId = request.empresaId || null;
    const batch = [];
    const uids = [request.requesterUid, request.targetUid].filter((u) => typeof u === 'string');
    for (const uid of uids) {
        batch.push(db.collection('user_notifications').add({
            uid,
...
async function applySwapToTurnos(db, request, supervisorName) {
    ...
    const dateA = dateKeyAr(shiftA.startTime);
    const dateB = dateKeyAr(shiftB.startTime);
    ...
    batch.update(db.collection('turnos').doc(myShiftId), {
        ...
        isSwap: true,
        swapAuthorized: true,
        swapAuthorizedAt: firestore_1.FieldValue.serverTimestamp(),
...
exports.getSwapPeople = functions.https.onCall(async (_data, context) => {
    const { empId, empData } = await assertPortalEmployee(context);
    const db = admin.firestore();
    const empresaId = String(empData.empresaId || '').trim();
    const now = firestore_1.Timestamp.now();
    const horizon = firestore_1.Timestamp.fromMillis(now.toMillis() + 45 * 24 * 60 * 60 * 1000);
...
exports.getSwapCandidates = functions.https.onCall(async (data, context) => {
    const { empId, empData } = await assertPortalEmployee(context);
    ...
    for (const doc of snap.docs) {
        if (doc.id === shiftId)
            continue;
        const dat = doc.data();
        if (dat.employeeId === empId)
            continue;
        if (!shiftIsSwappable(dat))
            continue;
        const eid = dat.employeeId || '';
...
exports.createSwapRequest = functions.https.onCall(async (data, context) => {
    ...
    if (objectiveId && targetShift.objectiveId !== objectiveId) {
        throw new functions.https.HttpsError('failed-precondition', 'Por ahora la permuta es solo dentro del mismo objetivo.');
    }
    const targetEmpSnap = await db.collection('empleados').doc(targetEmpId).get();
    const targetUid = targetEmpSnap.data()?.uid;
    ...
    const ref = await db.collection('swap_requests').add({
        ...
        targetShiftId,
        requesterShiftDate: dateKeyAr(myShift.startTime),
        targetShiftDate: dateKeyAr(targetShift.startTime),
    ...
    await notifySwapParties(db, {
        requesterUid: uid,
        targetUid,
        requesterId: empId,
        targetId: targetEmpId,
        empresaId: myShift.empresaId,
    }, 'Solicitud de permuta', `${requesterName} quiere permutar turno contigo. Revisá en la app.`);
    return { success: true, requestId: ref.id };
});
exports.respondSwapRequest = functions.https.onCall(async (data, context) => {
    ...
    await ref.update({
        ...
        peerAcceptedAt: firestore_1.FieldValue.serverTimestamp(),
        updatedAt: firestore_1.FieldValue.serverTimestamp(),
    });
    await notifySwapParties(db, req, 'Permuta aceptada por compañero', `${req.targetName || 'Tu compañero'} aceptó. Confirmá la permuta en la app.`);
    return { success: true, status: 'PENDING_REQUESTER' };
});
exports.confirmSwapRequest = functions.https.onCall(async (data, context) => {
    const { uid, empId } = await assertPortalEmployee(context);
    const requestId = String(data?.requestId || '').trim();
    ...
    await ref.update({
        ...
        requesterConfirmedAt: firestore_1.FieldValue.serverTimestamp(),
        updatedAt: firestore_1.FieldValue.serverTimestamp(),
    });
    await notifySwapParties(db, req, 'Permuta pendiente de supervisor', 'Ambos guardias confirmaron. Un supervisor debe autorizar en planificación.');
    return { success: true, status: 'PENDING_SUPERVISOR' };
});
exports.cancelSwapRequest = functions.https.onCall(async (data, context) => {
    const { uid, empId } = await assertPortalEmployee(context);
    const requestId = String(data?.requestId || '').trim();
    if (!requestId) {
        throw new functions.https.HttpsError('invalid-argument', 'requestId requerido.');
...
async function assertSupervisorOrAdmin(context) {
    if (!context.auth?.uid) {
        throw new functions.https.HttpsError('unauthenticated', 'Login requerido.');
    }
...
exports.approveSwapRequest = functions.https.onCall(async (data, context) => {
    ...
    await notifySwapParties(db, req, 'Permuta autorizada', 'Un supervisor autorizó la permuta. Los turnos ya están actualizados.');
    return { success: true, status: 'APPROVED' };
});
exports.rejectSwapRequestSupervisor = functions.https.onCall(async (data, context) => {
    ...
    await notifySwapParties(db, req, 'Permuta rechazada', 'Un supervisor rechazó la permuta. Los turnos no se modificaron.');
    return { success: true, status: 'REJECTED' };
});
//# sourceMappingURL=swapPortal.js.map