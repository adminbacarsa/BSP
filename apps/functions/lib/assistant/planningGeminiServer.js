...
function buildUserPrompt(context) {
    const topeHsPorPuesto = Object.fromEntries((context.puestos || []).map((p) => [p.positionName, maxHorasFacturablesDiaPorPuesto(p)]));
...
CICLOS_AUTORIZADOS (autoCycles seleccionados): ${context.autoCycles ? JSON.stringify(context.autoCycles.map((c) => c.id || c.name || c)) : '[]'}
...
${context.puestos.map((p) => {
        const billable = (p.shifts || []).map((s) => s.code).join(', ') || 'M';
...
async function runPlanningGeminiOptimize(context) {
    ...
    if (!match) {
        throw new functions.https.HttpsError('internal', `Gemini no devolvió JSON válido: ${text.slice(0, 300)}`);
    }
    try {
...
//# sourceMappingURL=planningGeminiServer.js.map