...
exports.slaHorasVendidasMesCalendario = slaHorasVendidasMesCalendario;
const WEEK_DAY_CODES = ['D', 'L', 'M', 'X', 'J', 'V', 'S'];
const SHIFT_VARIANTS_DB = {
...
function computePositionDayComposition(pos, dayCode) {
    let dayTotal = 0;
    let dayNight = 0;
    const addVariant = (v) => {
...
function slaHorasVendidasMesCalendario(positions, contractStartYmd, contractEndYmd, refYmd) {
    ...
    while (cur <= end) {
        ...
        for (const pos of posList) {
            const p = pos;
            const activeDays = Array.isArray(p.activeDays) ? p.activeDays : null;
            if (activeDays && activeDays.length > 0 && !activeDays.includes(dayCode))
                continue;
...
//# sourceMappingURL=assistantSlaHours.js.map