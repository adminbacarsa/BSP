...
async function ejecutarPlanificarObjetivoMes(empresaId, payload) {
    const { objetivoId, year, month, objetivoNombre } = payload;
    if (!objetivoId || !year || !month)
        throw new Error('Payload incompleto para planificar_objetivo_mes.');
    const { runPlanningAutomationCycle } = await Promise.resolve().then(() => require('../automation/operationalAutomation'));
