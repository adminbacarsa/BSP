...
exports.ASSISTANT_FUNCTION_DECLARATIONS.push({
    ...
    description: 'Resume alertas operativas generadas automáticamente por IA (marcaciones tardías, turnos vencidos, solapamientos, ausencias sin cobertura) para hoy o una fecha dada. Usá para «qué alertas raras hay», «alertas IA pendientes», «anomalías operativas».',
    parameters: {
        type: generative_ai_1.SchemaType.OBJECT,
        properties: {
            fecha: { type: generative_ai_1.SchemaType.STRING, description: 'YYYY-MM-DD opcional. Si se omite, usa hoy del cliente.' },
            solo_pendientes: { type: generative_ai_1.SchemaType.BOOLEAN, description: 'true (default): solo status pending.' },
            limite: { type: generative_ai_1.SchemaType.NUMBER, description: 'Máximo de filas de muestra (default 40).' },
        },
        required: [],
    },
}, {
    name: 'checklist_cierre_operativo_mes',
    description: 'Genera checklist operativo de cierre (liquidación + prefactura): marcaciones pendientes, turnos abiertos vencidos, ausencias sin resolver, horas SLA vendidas vs planificadas vs ejecutadas y recomendación de cierre. Usá para «cómo vengo para cerrar mayo», «control de prefactura cierre», «faltantes de liquidación».',
    parameters: {
        type: generative_ai_1.SchemaType.OBJECT,
        properties: {
            anio: { type: generative_ai_1.SchemaType.NUMBER, description: 'Año del cierre (ej. 2026).' },
            mes: { type: generative_ai_1.SchemaType.NUMBER, description: 'Mes 1-12 (ej. mayo = 5).' },
            persistir_snapshot: { type: generative_ai_1.SchemaType.BOOLEAN, description: 'true por default: guarda snapshot en operational_closure_checklists.' },
        },
        required: [],
    },
}, {
    name: 'recomendar_cobertura_vacante',
    description: 'Recomienda candidatos de cobertura ordenados por costo/riesgo CCT (SIN_TURNO→RET→ESC→…→FT) para una vacante o ausencia. Usá para «quién conviene para cubrir a García», «mejor opción de cobertura», «candidatos baratos para la vacante».',
    parameters: {
        type: generative_ai_1.SchemaType.OBJECT,
        properties: {
            shift_id: { type: generative_ai_1.SchemaType.STRING, description: 'ID Firestore del turno vacante/ausente (preferido).' },
            id_objetivo: { type: generative_ai_1.SchemaType.STRING, description: 'ID del objetivo si no hay shift_id.' },
            texto_objetivo: { type: generative_ai_1.SchemaType.STRING, description: 'Nombre del objetivo si no hay id.' },
            fecha: { type: generative_ai_1.SchemaType.STRING, description: 'YYYY-MM-DD. Default: hoy cliente o fecha del turno.' },
            banda: { type: generative_ai_1.SchemaType.STRING, description: 'Código de banda (M/T/N/D12/N12). Default: del turno.' },
            limite: { type: generative_ai_1.SchemaType.NUMBER, description: 'Máximo de candidatos (default 12).' },
        },
        required: [],
    },
}, {
    name: 'ejecutar_replan_diario',
    description: 'Ejecuta replanificación diaria rolling window (P1): detecta vacantes/ausencias en los próximos N días y recomienda cobertura. Con simulacion=true (default) solo recomienda y registra automation_runs. Con simulacion=false y aplicar_ret=true puede crear borradores RET de cobertura. Usá para «replan de los próximos días», «qué vacantes vienen», «armar cobertura preventiva».',
    parameters: {
        type: generative_ai_1.SchemaType.OBJECT,
        properties: {
            dias_ventana: { type: generative_ai_1.SchemaType.NUMBER, description: 'Días hacia adelante (1-14, default 3).' },
            id_objetivo: { type: generative_ai_1.SchemaType.STRING, description: 'Opcional: limitar a un objetivo.' },
            texto_objetivo: { type: generative_ai_1.SchemaType.STRING, description: 'Nombre del objetivo si no hay id.' },
            simulacion: { type: generative_ai_1.SchemaType.BOOLEAN, description: 'true (default): no escribe turnos. false: permite aplicar.' },
            aplicar_ret: { type: generative_ai_1.SchemaType.BOOLEAN, description: 'Si simulacion=false, crear borradores RET del mejor candidato RET.' },
            max_vacantes: { type: generative_ai_1.SchemaType.NUMBER, description: 'Tope de vacantes a procesar (default 40).' },
        },
        required: [],
    },
});
