...
function looksLikeObjectiveHintFalsePositive(hint) {
    ...
    if (/\bpor\s+empleado\b/.test(h))
        return true;
    if (/\b(cuantas|cuántas|total|cantidad|numero|número)\s+horas?\b/.test(h))
        return true;
    if (/^(cuantas|cuántas|total|cantidad|de\s+cuantas|de\s+cuántas)\s+horas?$/i.test(h.trim()))
        return true;
    if (h.length < 4)
        return true;
    return false;
...
function extractHourStartFilterFromQuery(t) {
    const n = normText(t);
...
function extractShiftBandCodeFromQuery(t) {
    const n = normText(t);
    if (/\b(turno\s+de\s+manana|turno\s+mañana|banda\s+m\b|codigo\s+m\b)\b/.test(n) && !extractHourStartFilterFromQuery(t)) {
        return 'M';
    }
    if (/\b(turno\s+tarde|banda\s+t\b|codigo\s+t\b)\b/.test(n))
        return 'T';
    if (/\b(turno\s+noche|banda\s+n\b|codigo\s+n\b)\b/.test(n))
        return 'N';
    return null;
}
function matchMyAssignedShiftsIntent(t) {
    return /\b(mis turnos|mi turno|tengo asignad|me asignaron|que turnos tengo|cuales son mis turnos|cuáles son mis turnos)\b/.test(t);
}
function matchObjectivePresentAtSiteIntent(t) {
    if (/\b(presentes?|fichados?|marcaron|marcacion|marcación|hay\s+ahora)\b/.test(t))
        return true;
    if (t.length > 90)
        return false;
...
function matchTurnosPlanificadosDiaIntent(t) {
    if (matchMyAssignedShiftsIntent(t))
        return false;
...
function extractMonthRangeFromHoursQuery(t, refYmd) {
    let year;
    try {
    ...
    for (const [name, mo] of Object.entries(SPANISH_MONTHS)) {
        if (new RegExp(`\\b${name}\\b`).test(t))
            return monthRangeYsMmDd(year, mo);
    }
    const ref = parseRefYmd(refYmd);
...
function looksLikeEmployeeNameFragment(frag) {
    const n = normText(frag);
    if (n.length < 3)
        return false;
    if (/^(horas?\s+)?(extras?|diurnas?|nocturnas?|reales?|teoricas?|liquidaci[oó]n|al\s*100|feriados?|bolsa|ft)\b/.test(n))
        return false;
    if (/\b(extras?|diurnas?|nocturnas?|liquidaci[oó]n)\b/.test(n) && n.split(/\s+/).length <= 3)
        return false;
    return true;
}
function extractEmployeeNameFromHoursQuery(raw) {
    ...
    if (m?.[1] && looksLikeEmployeeNameFragment(m[1]))
        return m[1].trim();
    ...
    if (m?.[1] && looksLikeEmployeeNameFragment(m[1]))
        return m[1].trim();
    m = t.match(new RegExp(`\\b(?:cuantas?|cuántas?)\\s+horas?\\s+trabaj\\w*\\s+([a-záéíóúñ][a-záéíóúñ\\s]{2,48}?)\\s+en\\s+(?:el\\s+)?(?:mes\\s+de\\s+)?(?:${monthNames})\\b`));
    if (m?.[1] && looksLikeEmployeeNameFragment(m[1]))
        return m[1].trim();
    return null;
}
function extractEmployeeNameFromTurnosDetailQuery(raw) {
    const t = normText(raw);
    let m = t.match(/\b(?:detalle|listado|mostrar|mostrame|mostrá|ver|pasame|pasa)\b.{0,40}\b(?:de\s+)?(?:los\s+|las\s+)?turnos?\s+(?:de|del)\s+(.+)$/);
    if (m?.[1] && looksLikeEmployeeNameFragment(m[1]))
        return m[1].trim();
...
function extractEmployeeNameFromRecentMessages(recent) {
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 10; i--) {
        const c = recent[i].content || '';
        const encontre = c.match(/Encontr[eé]\s+a\s+\*\*([^*]+)\*\*/i);
        if (encontre?.[1] && looksLikeEmployeeNameFragment(encontre[1]))
            return encontre[1].trim();
        const boldMatches = c.match(/\*\*([^*]{3,64})\*\*/g);
        if (boldMatches) {
            for (const b of boldMatches) {
                const name = b.replace(/\*\*/g, '').trim();
                if (!looksLikeEmployeeNameFragment(name))
                    continue;
                if (/^(reportes|planificaci[oó]n|operaciones|firestore|servicios|rrhh|dashboard)$/i.test(name))
                    continue;
                if (name.split(/[\s,]+/).filter(Boolean).length >= 2 || /,/.test(name))
                    return name;
            }
        }
        if (recent[i].role === 'user') {
            const frag = extractEmployeeNameFromHoursQuery(c) ||
...
function matchEmployeeTurnosDetailIntent(t, recent) {
    if (!/\b(turnos?)\b/.test(t))
        return false;
    const wantsDetail = /\b(detalle|listado|mostrar|mostrame|mostrá|ver|pasame|pasa|desglos|itemizado)\b/.test(t) ||
    ...
    if (extractEmployeeNameFromHoursQuery(t) || extractEmployeeNameFromTurnosDetailQuery(t))
        return true;
    return !!extractEmployeeNameFromRecentMessages(recent);
}
function formatCordobaDayFromIso(iso) {
    if (!iso)
        return '—';
    try {
...
function formatTurnosDetailReply(nombre, rangeLabel, data) {
...
function matchLiquidacionEmpresaIntent(t, recent = []) {
    ...
    if (/\b(cantidad|total)\b/.test(t) && /\b(liquidar|liquidaci)\b/.test(t))
        return true;
    return false;
}
function matchLiquidacionEmpresaIntentExcludeEmployee(t) {
    if (!/\bhoras?\b/.test(t))
        return false;
...
function formatLiquidacionEmpresaReply(agg, rangeLabel) {
    ...
    const muestra = (agg.muestra_empleados ?? []);
    if (muestra.length > 0) {
        ...
        for (const e of muestra.slice(0, 6)) {
            ...
            if (Number(e.al_100_ft) > 0)
                body += `, ${e.al_100_ft} h al 100%`;
            body += ')\n';
        }
    }
    if (trunc) {
...
async function tryDeterministicLiquidacionEmpresaReply(t, toolCtx, recent = []) {
    if (!matchLiquidacionEmpresaIntent(t, recent))
        return null;
    ...
    const resumen = await (0, assistantDataTools_1.ejecutarResumenHorasLiquidacionEmpresaPeriodo)(toolCtx, {
...
function matchEmployeeHoursPeriodIntent(t, recent = []) {
    if (matchLiquidacionEmpresaIntent(t, recent))
        return false;
    ...
    const hasPeriod = Object.keys(SPANISH_MONTHS).some((name) => new RegExp(`\\b${name}\\b`).test(t)) ||
        /\b(este mes|mes actual|mes pasado|mes anterior|20\d{2}-\d{2}-\d{2})\b/.test(t);
    if (!hasPeriod)
        return false;
    if (/\b(?:de|del)\s+\w/.test(t))
        return true;
    return new RegExp(`${RE_HORA}.{0,40}\\b[a-záéíóúñ]{3,}\\s+[a-záéíóúñ]{3,}`).test(t);
}
function formatDeterministicHoursReply(nombre, rangeLabel, resumen) {
...
async function tryDeterministicEmployeeTurnosDetailReply(raw, t, toolCtx, recent) {
    if (!matchEmployeeTurnosDetailIntent(t, recent))
        return null;
    ...
    const search = await (0, assistantDataTools_1.ejecutarBuscarEmpleadosPorNombre)(toolCtx, { texto: nameFragment, limite: 6 });
    ...
    if (!empId)
        return null;
    const detalle = await (0, assistantDataTools_1.ejecutarConsultarTurnosEmpleado)(toolCtx, {
...
async function tryDeterministicEmployeeHoursReply(raw, t, toolCtx, recent = []) {
    if (!matchEmployeeHoursPeriodIntent(t, recent))
        return null;
    const nameFragment = extractEmployeeNameFromHoursQuery(raw) || extractEmployeeNameFromRecentMessages(recent);
    let range = extractMonthRangeFromHoursQuery(t, toolCtx.referenceDateYsMmDd);
    if (!range) {
        range = extractMonthRangeFromHoursQuery(recent
            .slice(-6)
    ...
    if (!empId)
        return null;
    const resumen = await (0, assistantDataTools_1.ejecutarResumenHorasEmpleadoPeriodo)(toolCtx, {
        id_firestore_empleado: empId,
        fecha_desde: range.desde,
        fecha_hasta: range.hasta,
    });
    ...
    const empBlock = resumen.empleado;
...
function recentTurnosHoyThread(recent) {
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 8; i--) {
        const tc = normText(recent[i].content || '');
        if (matchWhoOnShiftTodayIntent(tc))
            return true;
...
function matchTurnosHoyFollowUpIntent(t, recent) {
    ...
    if (matchTurnosPlanificadosDiaIntent(t))
        return true;
    if (matchWhoOnShiftDayIntent(t))
        return true;
    if (/^(si|sí|dale|ok|bueno|perfecto)\.?$/i.test(t.trim()) && recentTurnosHoyThread(recent))
        return true;
    if (/\b(no hay nadie|nadie trabaja|esta vacio|está vacío|sin nadie|no hay turno|no hay guardia|hay alguien|alguien trabaja)\b/.test(t) &&
        recentTurnosHoyThread(recent)) {
        return true;
    }
    return false;
}
function extractFechaFromRecentTurnosThread(recent, fallback) {
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 8; i--) {
...
function recentFrancoRetThread(recent) {
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 10; i--) {
        const c = normText(recent[i].content || '');
        if (/\b(ausentes?\s+en\s+operaciones|faltaron|licencias?\s+en\s+rrhh)\b/.test(c))
            return false;
        if (/\b(franco|ret\b|reten)\b/.test(c))
            return true;
        if (/\b(codigos? de legajo|empleados con franco|en ret)\b/.test(c))
            return true;
        if (/\bresumen_por_objetivo\b/.test(recent[i].content || ''))
            return true;
    }
    return false;
}
function recentAbsenceAggregateThread(recent) {
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 8; i--) {
        const c = recent[i].content || '';
        if (!/\b(ausentes?|faltaron|licencias?)\b/i.test(c))
            continue;
        const aus = c.match(/\*\*(\d+)\*\*\s+ausentes/i) || c.match(/(\d+)\s+ausentes/i);
        if (aus?.[1]) {
            const iso = c.match(/\b(20\d{2}-\d{2}-\d{2})\b/);
    ...
    if (ops && ops.ausentes > 0)
        return { ausentes: ops.ausentes, fecha: ops.fecha };
    return null;
}
function recentAbsenceThread(recent) {
    if (recentAbsenceAggregateThread(recent))
        return true;
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 10; i--) {
        const c = normText(recent[i].content || '');
        if (/\b(ausentes?\s+en\s+operaciones|faltaron|licencias?\s+el|de\s+licencia)\b/.test(c))
            return true;
    }
    return false;
}
function matchAbsenceFollowUpIntent(t, recent) {
    if (recentFrancoRetThread(recent) && !recentAbsenceThread(recent))
        return false;
    const trimmed = t.trim();
    const absenceCtx = recentAbsenceThread(recent);
    const opsCtx = parseOpsAggregateFromRecent(recent);
    if (!absenceCtx && !(opsCtx && opsCtx.ausentes > 0))
        return false;
    if (/^(quienes|quiénes|quien|quién)\s*(son|es|estan|están)?\.?$/i.test(trimmed))
        return true;
    if (/\b(quienes|quiénes)\s+(son|estan|están|es)\b/.test(t))
        return true;
    if (/\b(nombres|nombralos|nombrarlos|decime los nombres)\b/.test(t))
        return true;
    if (/\b(ausencias?|ausentes?|faltaron|licencias?)\b/.test(t) && /\b(dame|list|mostr|decime|podes|podés)\b/.test(t)) {
        return true;
    }
    const m = t.match(/\b(?:son\s+)?(?:los?\s+)?(\d+)\b/);
    if (m?.[1] && opsCtx && Number(m[1]) === opsCtx.ausentes)
        return true;
    return false;
}
function matchAbsenceOrLicenseDayIntent(t, recent) {
    if (matchAbsenceFollowUpIntent(t, recent))
        return true;
    if (/\bfranco\b/.test(t) && !/\b(ausent|falt|licencia|enfermedad|vacacion)\b/.test(t))
        return false;
    const hasAbsenceKw = /\b(faltaron|falto|faltó|falta|ausent|inasisten|no\s+se\s+present|licencia|enfermedad|vacacion|vacaciones)\b/.test(t);
    if (!hasAbsenceKw) {
    ...
    if (/\b(hoy|ayer|manana|mañana|el dia|el día|este dia|este día|20\d{2}-\d{2}-\d{2})\b/.test(t))
        return true;
    const blob = recent.slice(-4).map((m) => m.content).join(' ');
    if (/\b(hoy|ayer|manana|mañana)\b/.test(normText(blob)))
        return true;
    return /\b(faltaron|faltó|falto|ausent)\b/.test(t);
}
function extractAbsenceTipoFromQuery(t, recent) {
    const blob = [t, ...recent.slice(-3).map((m) => m.content)].join(' ');
    const n = normText(blob);
    const licencia = /\b(licencia|enfermedad|vacacion|vacaciones|art\b)\b/.test(n) && !/\b(ausent|faltaron|faltó|falto)\b/.test(t);
    const ausente = /\b(faltaron|faltó|falto|falta|ausent|inasisten|no\s+se\s+present)\b/.test(n);
    if (licencia && !ausente)
        return 'licencias';
    if (ausente && !licencia)
        return 'ausentes';
    if (/\bde\s+licencia\b/.test(n))
        return 'licencias';
    if (/\bausentes?\b/.test(t) && !/\blicencia\b/.test(t))
        return 'ausentes';
    return 'ambos';
}
async function tryDeterministicAusentesLicenciasDiaReply(t, toolCtx, recent) {
    if (!matchAbsenceOrLicenseDayIntent(t, recent))
        return null;
    const fecha = (recentAbsenceAggregateThread(recent)?.fecha && matchAbsenceFollowUpIntent(t, recent)
    ...
    if (siteHint && siteHint.length >= 3) {
        ...
        if (coincidencias.length === 1)
            idObj = coincidencias[0].id_objetivo;
    }
    const data = await (0, assistantDataTools_1.ejecutarListadoAusentesLicenciasDia)(toolCtx, {
        fecha,
        tipo,
        limite: 120,
        id_objetivo: idObj,
    ...
    return (0, assistantDataTools_1.formatListadoAusentesLicenciasParaChat)(data).slice(0, 7500);
}
function matchFrancoRetFollowUpIntent(t, recent) {
    if (recentAbsenceThread(recent) && !recentFrancoRetThread(recent))
        return false;
    const trimmed = t.trim();
    if (/^(cuantos|cuántas?)\s+son\.?$/i.test(trimmed) && recentFrancoRetThread(recent))
        return true;
    if (/^(quienes|quiénes|quien|quién)\s*(son|es|estan|están)?\.?$/i.test(trimmed)) {
        return recentFrancoRetThread(recent);
    }
    if (/\b(quienes|quiénes)\s+(son|estan|están|es)\b/.test(t) && recentFrancoRetThread(recent))
        return true;
    if (/\b(nombres|nombralos|nombrarlos|decime los nombres)\b/.test(t) && recentFrancoRetThread(recent))
        return true;
    return false;
}
function matchFrancoRetDiaIntent(t, recent) {
    if (matchAbsenceOrLicenseDayIntent(t, recent))
        return false;
    if (/\b(faltaron|falto|faltó|ausent|licencia|enfermedad|vacacion|vacaciones|inasisten)\b/.test(t))
        return false;
    if (matchFrancoRetFollowUpIntent(t, recent))
        return true;
    if (!/\b(franco|ret\b|reten)\b/.test(t))
        return false;
    if (/\b(quien|quién|quienes|quiénes|cuantos|cuántos|lista|hay|cuenta)\b/.test(t))
        return true;
    if (/\b(codigo|cod)\s*f\b/.test(t))
        return true;
    return false;
}
function extractFrancoRetTipoFromQuery(t, recent) {
...
function extractObjectiveSiteFromQuery(t) {
    ...
    if (m?.[1]?.trim()) {
        const frag = m[1].trim();
        if (!/\b(franco|ret|turno|tarde|mañana|noche|personas?|cuantos|cuántos)\b/i.test(frag))
            return frag;
    }
...
async function tryDeterministicFrancoRetDiaReply(t, toolCtx, recent) {
    ...
    const data = await (0, assistantDataTools_1.ejecutarListadoFrancoRetDia)(toolCtx, listadoArgs);
    const err = String(data.error ?? '').trim();
    if (err) {
        if (err === 'sin_permiso_requiere_modulo_operaciones_planificacion_o_similar') {
    ...
    return (0, assistantDataTools_1.formatListadoFrancoRetParaChat)(data).slice(0, 7500);
}
function matchTurnosObjetivoHoyIntent(t) {
    if (!/\b(hoy|el dia|el día)\b/.test(t))
        return false;
    if (!/\b(turnos?|tarde|mañana|noche|personas?|alguien|viene|vienen)\b/.test(t))
        return false;
    if (/\b(en|del|de)\s+(?:el\s+)?(?:h\.?|hospital|[a-záéíóúñ])/i.test(t))
        return true;
    if (/\bturnos?\s+(?:del?\s+|de\s+(?:el\s+)?)?(?:h\.?\s*|hospital\s*)?[a-záéíóúñ0-9]/i.test(t))
        return true;
    return false;
}
function matchWhoOnShiftTodayIntent(t) {
    return matchWhoOnShiftDayIntent(t);
}
function matchWhoOnShiftDayIntent(t) {
    if (/\b(franco|ret\b|reten|cercan|faltaron|falto|faltó|ausent|licencia|enfermedad|vacacion)\b/.test(t)) {
        return false;
    }
    if (matchTurnosObjetivoHoyIntent(t))
        return true;
    const hasDay = /\b(hoy|manana|mañana|ayer|el dia|el día|este dia|este día|20\d{2}-\d{2}-\d{2}|\d{1,2}\/\d{1,2})\b/.test(t) ||
        extractHourStartFilterFromQuery(t) != null;
    if (!hasDay && !/\bturno\s+hoy\b/.test(t))
        return false;
    if (/\b(quien|quién|quienes|quiénes)\b/.test(t) && /\b(turno|trabaja|trabajan|guardia|guardias|personal|esta|están|hay)\b/.test(t)) {
        return true;
    }
    if (/\bturno\s+hoy\b/.test(t))
        return true;
    if (/\b(hoy|manana|mañana)\b/.test(t) && /\b(tiene|tienen|esta|están|hay)\b/.test(t))
        return true;
    if (extractHourStartFilterFromQuery(t) && /\b(quien|quién|quienes|quiénes|turno|trabaja)\b/.test(t))
        return true;
    return false;
}
function shouldSkipDeterministicRouter(raw, recent = []) {
    ...
    if (matchMyAssignedShiftsIntent(t))
        return false;
    if (matchObjectivePresentAtSiteIntent(t))
        return false;
    if (matchOpsTurnoCountFollowUpIntent(t, recent))
        return false;
    if (matchTurnosPlanificadosDiaIntent(t))
    ...
    if (matchFrancoRetDiaIntent(t, recent))
        return false;
    if (matchAffirmativeShortIntent(t) && recentOffersSlaAllObjectivesBatch(recent))
        return false;
    if (raw.length > 240)
        return true;
...
function matchEmployeeCountIntent(t, moduleKey, pathname, recent = []) {
...
function matchClientListIntent(t, recent) {
    ...
    if (recentClientListThread(recent)) {
        if (/\b(demas|restantes|completa|otros|lista|listado|nombres?|faltan|21|10|muestras?|cuales?\s+son|listame|nombralos)\b/.test(t)) {
            return true;
        }
    }
    return false;
...
function extractClientNameFromQuery(t) {
    ...
    if (m2?.[1] && !/\b(cuántos|cuantos|objetivos?|sedes?)\b/i.test(m2[1]))
        return m2[1].trim();
...
function matchClientObjectivesIntent(t) {
    if (/\bobjetivos?\b/.test(t) && /\b(cliente|empresa)\b/.test(t))
        return true;
    if (/\bobjetivos?\b/.test(t) && extractClientNameFromQuery(t))
        return true;
    if (/\b(sedes?|ubicaciones?)\b/.test(t) && extractClientNameFromQuery(t))
        return true;
...
function matchHoursToPlanIntent(t) {
    return (/\bhoras?\s+(?:a\s+)?planif/.test(t) ||
        /\b(?:cuantas|cuántas)\s+horas\b.{0,28}\bplanif/.test(t) ||
        /\bhoras?\s+vendid/.test(t) ||
        /\bfalta\s+planif/.test(t));
}
function matchSlaHoursQueryIntent(t, recent = []) {
    if (matchHoursToPlanIntent(t))
        return true;
    if (/\bhoras?\s+(?:del|de)\s+(?:el\s+)?(?:servicio|contrato|sla)\b/.test(t))
        return true;
    if (/\btotal\s+de\s+horas?\b/.test(t) && /\b(servicio|contrato|sla)\b/.test(t))
        return true;
    ...
    if (/\bel\s+servicio\s+sla\b/.test(t) && /\bhoras?\b/.test(t))
        return true;
    if (recentMentionsSlaOrContrato(recent)) {
        if (/\b(de\s+)?(cuantas|cuántas|total|cantidad)\b/.test(t) && /\bhoras?\b/.test(t))
            return true;
        if (/^(?:y\s+)?(?:las?\s+)?horas?\s*\??\s*$/i.test(t.trim()))
            return true;
    }
    return false;
}
function recentMentionsSlaOrContrato(recent) {
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 8; i--) {
        ...
        if (/\b(hay\s+\d+\s+(servicio|sla|contrato)s?\s+activ|servicio\s+activo|sla\s+activo)\b/.test(t))
            return true;
        if (/\bobjetivo\s+[a-záéíóúñ0-9]/i.test(recent[i].content || '')) {
            if (/\b(servicio|contrato|sla|ministro|misericordia)\b/.test(t))
                return true;
        }
        if (/\ben\s+(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\b/.test(t)) {
            if (/\b(servicio|contrato|sla|kpi)\b/.test(t))
                return true;
        }
        if (/\bcliente\s*[-–]\s*objetivo\b/.test(t) || /\bcasisa\b/.test(t))
            return true;
    }
    return false;
}
function extractClienteObjetivoPairsFromContent(c) {
    const pairs = [];
    const seen = new Set();
    const add = (clienteRaw, objetivoRaw) => {
...
function extractClienteObjetivoPairsFromRecent(recent) {
    const all = [];
    const seen = new Set();
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 10; i--) {
...
function matchMultiSlaFromListIntent(t, recent) {
    ...
    if (/\b(cada uno|cada una|todos esos|esos servicios|esos contratos|de la lista|los listados|los cuatro|los 4)\b/.test(t)) {
        if (/\b(sla|horas?|contrato|servicio|planif|vendid|detalle)\b/.test(t))
            return true;
    }
    if (/\b(que|qué)\s+sla\b/.test(t) && /\b(cada|cual|todos)\b/.test(t))
        return true;
    if (/\bhoras?\b/.test(t) && /\b(cada|todos|esos)\b/.test(t))
        return true;
    return false;
}
function matchAffirmativeShortIntent(t) {
    return /^(si|sí|ok|dale|bueno|perfecto|por favor|porfa|de acuerdo|esta bien|está bien|mostrame|mostrá|mostrar|si por favor)\.?$/i.test(t.trim());
}
function recentOffersSlaAllObjectivesBatch(recent) {
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 8; i--) {
        if (recent[i].role !== 'assistant')
            continue;
        const n = normText(recent[i].content || '');
        if (!/\b(sla|servicios activos)\b/.test(n))
            continue;
        if (/\b(horas vendidas|ya planificadas|pendientes a planificar|planificadas en turnos)\b/.test(n) &&
            /\b(todos los objetivos|todos los servicios|servicios activos)\b/.test(n)) {
            return true;
        }
        if (/\b(te gustaria|te gustaría|quieres|queres|puedo mostrarte|resumen)\b/.test(n) &&
...
function matchSlaContractHoursIntent(t, recent) {
    ...
    if (/^(?:y\s+)?(?:las\s+)?horas?\s*\??\s*$/i.test(t.trim()) && recentMentionsSlaOrContrato(recent))
        return true;
    return false;
}
function extractObjectiveHintFromRecentMessages(recent) {
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 8; i--) {
        const m = recent[i];
        const c = m.content || '';
        const boldPair = c.match(/\*\*([^*]+)\*\*\s*[-–]\s*([^\n*]+)/);
        if (boldPair?.[2])
            return boldPair[2].trim();
        const listPair = c.match(/(?:^|\n)\s*[*•\-]+\s*([A-Za-zÁÉÍÓÚÑ0-9][A-Za-zÁÉÍÓÚÑ0-9\s]{1,40}?)\s*[-–]\s*([A-Za-zÁÉÍÓÚÑ][^\n(]+?)(?:\s*\(|\.|,|\n|$)/im);
        if (listPair?.[2])
            return listPair[2].trim();
        const plainPair = c.match(/\b(?:servicio|contrato)\s+activo\s+es:?\s*\n?\s*[-*]?\s*([^\n]+)\s*[-–]\s*([^\n]+)/i);
        if (plainPair?.[2])
            return plainPair[2].trim();
        const anyPair = c.match(/\b([A-Z][A-Z0-9\s]{2,36})\s*[-–]\s*([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ0-9\s]{4,56})/);
        if (anyPair?.[2] && /\b(contrato|servicio|sla|junio|mayo)\b/i.test(c))
            return anyPair[2].trim();
        if (anyPair?.[1] && anyPair?.[2])
            return `${anyPair[1].trim()} ${anyPair[2].trim()}`;
        const soloObj = c.match(/\bobjetivo\s+([A-ZÁÉÍÓÚÑ0-9][^\n.,]{4,60})/i);
        if (soloObj?.[1])
            return soloObj[1].trim();
    }
    return null;
}
function extractMonthRefYmdFromRecentMessages(recent, fallbackYmd) {
    for (let i = recent.length - 1; i >= 0 && i >= recent.length - 8; i--) {
        const t = normText(recent[i].content || '');
        const range = extractMonthRangeFromHoursQuery(t, fallbackYmd);
        if (range)
            return `${range.desde.slice(0, 7)}-15`;
    }
    return fallbackYmd;
}
...
function matchEmpleadosSinTurnosPlanificadosIntent(t) {
    if (/\b(mas\s+de|más\s+de|supera|umbral|200)\b/.test(t) && /\bhoras?\b/.test(t))
        return false;
    if (/\b(a\s+)?qu[eé]\s+colaboradores?\b/.test(t) &&
        /\b(no\s+les|sin\s+turno|ningun|ningún|no\s+planif)\b/.test(t)) {
        return true;
    }
    return (/\b(sin\s+turno|sin\s+planif|no\s+les?\s+planif|ningun\s+turno|ningún\s+turno|no\s+tienen?\s+turno|sin\s+nada\s+en\s+la\s+grilla)\b/.test(t) && /\b(planif|turno|colaborador|empleado|vigilador|guardia|nomina|nómina|legajo)\b/.test(t));
}
function formatDeterministicEmpleadosSinTurnosReply(data, rangeLabel) {
    ...
    const lista = (data.muestra_empleados_sin_turno ?? []);
    const trunc = data.truncado_por_limite_muestra === true ||
...
async function tryDeterministicEmpleadosSinTurnosReply(t, toolCtx, recent) {
    if (!matchEmpleadosSinTurnosPlanificadosIntent(t))
        return null;
    let range = extractMonthRangeFromHoursQuery(t, toolCtx.referenceDateYsMmDd);
    if (!range) {
        range = extractMonthRangeFromHoursQuery(recent
    ...
    const data = await (0, assistantDataTools_1.ejecutarListadoEmpleadosSinTurnosPlanificados)(toolCtx, {
        fecha_desde: range.desde,
        fecha_hasta: range.hasta,
        limite: 60,
    });
...
function formatMultiSlaHoursReply(batch, clientFilter) {
    ...
    for (const r of resultados) {
        ...
        if (puestos > 0)
            body += ` (${puestos} puesto(s) en contrato)`;
        body += `\n- **Ya planificadas:** **${plan}** h`;
        if (pend > 0)
            body += ` | **Pendiente a planificar:** **${pend}** h`;
        body += '\n\n';
    }
    const fail = Number(batch.con_error ?? 0);
    if (fail > 0)
        body += `*${fail} sitio(s) no se pudieron resolver; revisá el nombre en **Servicios y SLA**.*\n`;
    body += 'Detalle de puestos y fechas del contrato: **Servicios y SLA**; grilla mensual: **Planificación y Turnos**.';
    return body.trim();
}
function formatDeterministicSlaHoursToPlanReply(resumen) {
    const obj = (resumen.objetivo ?? {});
    const tot = (resumen.totales ?? {});
...
function formatDeterministicTurnosHoyReply(fecha, r) {
    ...
    for (const [site, lines] of bySite) {
        ...
        for (const ln of lines.slice(0, 12))
            body += `- ${ln}\n`;
        if (lines.length > 12)
            body += `- … y ${lines.length - 12} más en este sitio (en la muestra).\n`;
        body += '\n';
    }
...
function formatDeterministicTurnosPlanificadosDiaReply(fecha, r, opsFollowUp, filterLabel) {
    const total = Number(r.cuenta_total_turnos_visibles ?? 0);
    const muestra = (r.muestra_turnos ?? []);
    const truncLista = r.muestra_truncada_vs_total === true;
    const truncQuery = r.truncado_limite_turnos_consultados === true;
...
async function tryDeterministicCrmReply(t, toolCtx, recent = []) {
    ...
    if (matchClientObjectivesIntent(t)) {
        ...
        if (!hint || hint.length < 2)
            return null;
        const r = await (0, assistantDataTools_1.ejecutarListarObjetivosCliente)(toolCtx, { texto_cliente: hint, limite: 48 });
        const err = String(r.error ?? '').trim();
        ...
        if (err)
            return null;
        if (r.ambigua === true) {
            const lista = (r.clientes_coincidentes ?? [])
        ...
        if (cuenta > objs.length)
            body += `\n*… y ${cuenta - objs.length} más (abrí CRM para la lista completa).*\n`;
        if (r.truncado_por_limite === true)
            body += '\n*Lista acotada en el chat.*\n';
        return body.trim();
    }
    return null;
}
function formatPresentesEnObjetivoReply(fecha, siteLabel, r) {
    const total = Number(r.cuenta_total_turnos_visibles ?? 0);
    const muestra = (r.muestra_turnos ?? []);
    if (total === 0) {
...
async function tryDeterministicMyShiftsReply(t, toolCtx, recent) {
    if (!matchMyAssignedShiftsIntent(t))
        return null;
    ...
    const fecha = extractDayYmdFromQuery(t, toolCtx.referenceDateYsMmDd, recent) || toolCtx.referenceDateYsMmDd;
    const detalle = await (0, assistantDataTools_1.ejecutarConsultarTurnosEmpleado)(toolCtx, {
        id_firestore_empleado: empId,
        fecha_desde: fecha,
        fecha_hasta: fecha,
    ...
    const nombre = String(detalle.empleado?.nombre_legible ?? 'Tu legajo');
...
async function tryDeterministicPresentesEnObjetivoReply(t, toolCtx, recent) {
    if (!matchObjectivePresentAtSiteIntent(t))
        return null;
    ...
    const r = await (0, assistantDataTools_1.ejecutarListadoTurnosOperativosDia)(toolCtx, {
        fecha,
        id_objetivo: idObj,
        limite: 48,
...
async function tryDeterministicTurnosHoyReply(t, toolCtx, recent) {
    ...
    let filterLabel = '';
    if (horaFiltro)
        filterLabel = `inicio **${horaFiltro}**`;
    else if (codigoFiltro)
        filterLabel = `código **${codigoFiltro}**`;
    let idObj;
    if (siteHint && siteHint.length >= 3) {
        const found = await (0, assistantDataTools_1.ejecutarBuscarObjetivosPorNombre)(toolCtx, { texto: siteHint, limite: 4 });
        const coincidencias = (found.coincidencias ?? []);
        if (coincidencias.length === 1)
            idObj = coincidencias[0].id_objetivo;
    }
    ...
    const reply = usePlanFormat
        ? formatDeterministicTurnosPlanificadosDiaReply(fechaOut, r, opsFollow
            ...
            : undefined, filterLabel || undefined)
...
function matchPlanningUiOnlyIntent(t, moduleKey) {
    if (moduleKey !== 'PLANNING' && moduleKey !== 'PLANNING_AI')
        return false;
...
function matchPlanificacionHowToIntent(t) {
    ...
    if (/\b(publicar|cronograma|cornograma|grilla|planificar|asignar\s+turno)\b/.test(t)) {
        if (/\b(como|cómo|donde|dónde|pasos|ayuda|tutorial|comom)\b/.test(t))
            return true;
    }
    if (/\b(comom?|publico|publicar)\b/.test(t) && /\b(cronograma|cornograma|grilla)\b/.test(t))
        return true;
    return false;
}
function matchPlanningAutomateIntent(t) {
...
function tryDeterministicPlanificacionHowToReply(t) {
    if (!matchPlanificacionHowToIntent(t))
        return null;
    const publish = /\b(publicar|publico|cronograma|cornograma|comom)\b/.test(t) ||
        (/\b(grilla)\b/.test(t) && /\b(como|cómo)\b/.test(t));
    if (publish) {
        return ('**Publicar un cronograma en COSP**\n\n' +
            ...
            'Tras publicar, los turnos planificados quedan visibles para operaciones y el portal del empleado según las reglas de la empresa.').slice(0, 7500);
    }
    const guide = (0, cospKnowledge_1.operationalGuideForModuleKey)('PLANNING');
    if (!guide.trim())
        return null;
...
function tryDeterministicPlanningUiReply(moduleKey) {
    ...
    const body = guide.replace(/^GUÍA OPERATIVA[^\n]*\n/, '').trim();
    return (`**Planificación en COSP**\n\n${body.slice(0, 1500)}\n\n` +
        `Para **quién trabaja hoy** o **horas vendidas vs planificadas** de un objetivo, decime cliente/objetivo y mes y lo consulto en Firestore. Para ver o editar la grilla completa, usá **Planificación y Turnos**.`).slice(0, 7500);
}
function shouldPrefetchMetricsSnapshot(lastUser, moduleKey, recentMessages = []) {
    ...
    if (matchPlanificacionHowToIntent(t))
        return false;
    if (matchPlanningUiOnlyIntent(t, typeof moduleKey === 'string' ? moduleKey.trim() || null : null))
        return false;
...
function shouldPrefetchOperationsMetricsInSnapshot(lastUser) {
    return matchOpsDayAggregateIntent(normText(lastUser));
}
async function tryDeterministicSlaSoldHoursAggregateReply(t, toolCtx, recent) {
    if (!matchSlaHoursQueryIntent(t, recent))
        return null;
    if (matchMultiSlaFromListIntent(t, recent) || matchSlaAllActiveServicesIntent(t, recent))
        return null;
    ...
    if (objHint && objHint.length >= 3) {
        const resumen = await (0, assistantDataTools_1.ejecutarResumenHorasObjetivoSlaPeriodo)(toolCtx, {
...
async function tryDeterministicDataReply(lastUser, toolCtx, toolsEnabled, moduleKey, pathname, recentMessages = []) {
    if (!toolsEnabled || toolCtx.persona !== 'SYSTEM' || !toolCtx.empresaId.trim())
        return null;
    const raw = lastUser.trim();
    const recent = recentMessages.slice(-8);
    if (!raw || shouldSkipDeterministicRouter(raw, recent))
        return null;
    const t = normText(raw);
    const mk = typeof moduleKey === 'string' && moduleKey.trim() ? moduleKey.trim() : null;
    try {
    ...
    try {
        ...
        if (presentesObj?.trim())
            return presentesObj.trim();
    }
    catch (e) {
    ...
    if (!shouldSkipDeterministicRouter(raw, recent)) {
        ...
        try {
            ...
            if (slaPlanReply?.trim())
                return slaPlanReply.trim();
        }
        catch (e) {
    ...
    if (wantEmp) {
        ...
        if (String(r.error ?? '').trim()) {
            blocks.push('No pude contar la nómina desde Firestore (revisá permiso en **RRHH**). La cifra oficial está en la tarjeta **Empleados en nómina** del **Panel principal**.');
        }
        else {
            ...
            blocks.push(`Según **Firestore** (legajos de la empresa), hay **${n}** empleado(s) en nómina con status activo (misma regla que la tarjeta **Empleados en nómina** del panel). Criterio amplio RRHH (incluye sin estado explícito): **${amplio}**.`);
