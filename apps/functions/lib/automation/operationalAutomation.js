...
async function scanOperationalAlertsForEmpresa(input) {
    ...
    const now = new Date();
    const start = firestore_1.Timestamp.fromDate(new Date(now.getTime() - lookbackHours * 3600000));
    const end = firestore_1.Timestamp.fromDate(new Date(now.getTime() + lookaheadHours * 3600000));
    const docs = await queryShiftsForWindow(empresaId, start, end, 2500);
    const shifts = docs.map((d) => ({ id: d.id, data: d.data() }));
